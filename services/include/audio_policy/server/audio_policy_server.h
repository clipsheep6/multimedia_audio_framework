...
private:
    ...
    std::mutex mutex_;
    std::unordered_map<AudioStreamType, std::shared_ptr<AudioManagerCallback>> umPolicyListenerCbs_;
    std::unordered_map<AudioStreamType, AudioInterrupt> umCurActiveInterrupts_;
};
} // namespace AudioStandard
} // namespace OHOS
