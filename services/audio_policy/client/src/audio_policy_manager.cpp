...
int32_t AudioPolicyManager::ConfigDistributedRoutingRole(AudioDeviceDescriptor *descriptor, CastType type)
{
    const sptr<IAudioPolicy> gsp = GetAudioPolicyManagerProxy();
    if (gsp == nullptr) {
...
int32_t AudioPolicyManager::SetDistributedRoutingRoleCallback(int32_t clientId,
        const std::shared_ptr<AudioDistributedRoutingRoleCallback> &callback)
{
    ...
    return gsp->SetDistributedRoutingRoleCallback(clientId, object);
}

int32_t AudioPolicyManager::UnsetDistributedRoutingRoleCallback(const int32_t clientId)
{
    const sptr<IAudioPolicy> gsp = GetAudioPolicyManagerProxy();
    if (gsp == nullptr) {
    ...
    return gsp->UnsetDistributedRoutingRoleCallback(clientId);
} 
