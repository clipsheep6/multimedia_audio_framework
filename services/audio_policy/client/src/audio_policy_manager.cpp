...
{
    if (audioPolicyClientCB == nullptr) {
        audioPolicyClientCB = std::make_shared<AudioPolicyClientStubImpl>();
    }

    return audioPolicyClientCB;
}
    ...
    const std::shared_ptr<AudioPreferredOutputDeviceChangeCallback> &callback)
{
    ...
    return gsp->RegisterPreferredOutputDeviceChangeCbClient(object,
    ...
    const std::shared_ptr<AudioPreferredInputDeviceChangeCallback> &callback)
{
    ...
    return gsp->RegisterPreferredInputDeviceChangeCbClient(object,
...
int32_t AudioPolicyManager::UnsetPreferredOutputDeviceChangeCallback(const int32_t clientId)
{
    const sptr<IAudioPolicy> gsp = GetAudioPolicyManagerProxy();
    if (gsp == nullptr) {
        AUDIO_ERR_LOG("UnsetPreferredOutputDeviceChangeCallback: audio policy manager proxy is NULL.");
        return -1;
    }
    return gsp->UnRegisterPreferredOutputDeviceChangeCbClient(
        static_cast<uint32_t>(AudioPolicyClientCode::ON_ACTIVE_OUTPUT_DEVICE_UPDATED));
}
...
int32_t AudioPolicyManager::UnsetPreferredInputDeviceChangeCallback()
{
    const sptr<IAudioPolicy> gsp = GetAudioPolicyManagerProxy();
    if (gsp == nullptr) {
        AUDIO_ERR_LOG("UnsetPreferredInputDeviceChangeCallback: audio policy manager proxy is NULL.");
        return -1;
    }
    return gsp->UnregisterPreferredInputDeviceChangeCbClient(
        static_cast<uint32_t>(AudioPolicyClientCode::ON_ACTIVE_INPUT_DEVICE_UPDATED));
}
    ...
    const std::shared_ptr<VolumeKeyEventCallback> &callback, API_VERSION api_v)
{
    ...
    return gsp->RegisterVolumeKeyEventCallbackClient(object,
