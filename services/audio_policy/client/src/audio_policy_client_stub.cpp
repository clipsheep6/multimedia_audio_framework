...
{
    std::unique_ptr<std::vector<sptr<AudioDeviceDescriptor>>> object =
        std::make_unique<std::vector<sptr<AudioDeviceDescriptor>>>();;
    int32_t size = data.ReadInt32();
    for (int32_t i = 0; i < size; i++) {
        object->push_back(AudioDeviceDescriptor::Unmarshalling(data));
    }

    SendEvent(AppExecFwk::InnerEvent::Get(
        static_cast<uint32_t>(AudioPolicyClientCode::ON_ACTIVE_OUTPUT_DEVICE_UPDATED), object));
    reply.WriteInt32(SUCCESS);
}
...
void AudioPolicyClientStub::HandlePreferredInputDeviceUpdated(MessageParcel &data, MessageParcel &reply)
{
    ...
    SendEvent(AppExecFwk::InnerEvent::Get(static_cast<uint32_t>(AudioPolicyClientCode::ON_ACTIVE_INPUT_DEVICE_UPDATED),
        object));
    reply.WriteInt32(SUCCESS);
}
...
{
    std::unique_ptr<std::vector<std::unique_ptr<AudioRendererChangeInfo>>> object =
        std::make_unique<std::vector<std::unique_ptr<AudioRendererChangeInfo>>>();;
    int32_t size = data.ReadInt32();
    while (size > 0) {
        ...
        object->push_back(move(rendererChangeInfo));
        size--;
    }
    SendEvent(AppExecFwk::InnerEvent::Get(static_cast<uint32_t>(AudioPolicyClientCode::ON_RENDERERSTATE_CHANGE),
        object));
    reply.WriteInt32(SUCCESS);
}
...
{
    std::unique_ptr<std::vector<std::unique_ptr<AudioCapturerChangeInfo>>> object =
        std::make_unique<std::vector<std::unique_ptr<AudioCapturerChangeInfo>>>();
    int32_t size = data.ReadInt32();
    while (size > 0) {
        ...
        object->push_back(move(capturerChangeInfo));
        size--;
    }
    SendEvent(AppExecFwk::InnerEvent::Get(static_cast<uint32_t>(AudioPolicyClientCode::ON_RENDERERSTATE_CHANGE),
        object));
