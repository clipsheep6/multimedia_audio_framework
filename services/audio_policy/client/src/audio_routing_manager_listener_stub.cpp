...
{
    :AppExecFwk::EventHandler(AppExecFwk::EventRunner::Create("AudioRoutingManager"));
    ...
    uint32_t code, MessageParcel &data, MessageParcel &reply, MessageOption &option)
{
    ...
    switch (code) {
        case ON_MIC_STATE_UPDATED: {
            ...
            Thread_->SendEvent(AppExecFwk::InnerEvent::Get(Set_MICROPHONE_MUTE, micStateChangeEvent));
            return AUDIO_OK;
        }
        default: {
...
void AudioRoutingManagerListenerStub::OnMicStateUpdated(const MicStateChangeEvent &micStateChangeEvent)
{
    ...
    Thread_ = std::make_unique<std::thread>(&AudioRoutingManagerListenerStub::OnMicStateUpdated, this);
...
void AudioPolicyServer::ProcessEvent(const AppExecFwk::InnerEvent::Pointer &event)
{
    uint32_t eventId = event->GetInnerEventId();
    switch (eventId) {
        case Set_MICROPHONE_MUTE: {
            Thread_->OnMicStateUpdated();
            break;
        }
        default:
            break;
