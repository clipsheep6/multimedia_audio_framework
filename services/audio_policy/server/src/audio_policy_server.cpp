    ...
    const AudioStreamType &streamInFocus)
{
    ...
    if (volLevelCheck) {
        for (auto it = audioPolicyClientProxyCBMap_.begin(); it != audioPolicyClientProxyCBMap_.end(); ++it) {
            std::shared_ptr<AudioPolicyClientProxy> volumeChangeCb = it->second;
...
std::shared_ptr<AudioPolicyClientProxy> AudioPolicyServer::GetAudioPolicyClientProxy(
    const int32_t id, const sptr<IRemoteObject> &object)
{
    std::shared_ptr<AudioPolicyClientProxy> proxy = audioPolicyClientProxyCBMap_[id];
    if (proxy == nullptr && object != nullptr) {
        proxy = std::make_shared<AudioPolicyClientProxy>(object);
        audioPolicyClientProxyCBMap_[id] = proxy;
    }
    return proxy;
}
...
int32_t AudioPolicyServer::SetRingerMode(AudioRingerMode ringMode, API_VERSION api_v)
{
    ...
    if (ret == SUCCESS) {
        for (auto it = audioPolicyClientProxyCBMap_.begin(); it != audioPolicyClientProxyCBMap_.end(); ++it) {
            std::shared_ptr<AudioPolicyClientProxy> ringerModeListenerCb = it->second;
...
int32_t AudioPolicyServer::SetMicrophoneMuteCommon(bool isMute, API_VERSION api_v)
{
    ...
    if (ret == SUCCESS && isMicrophoneMute != isMute) {
        for (auto it = audioPolicyClientProxyCBMap_.begin(); it != audioPolicyClientProxyCBMap_.end(); ++it) {
            std::shared_ptr<AudioPolicyClientProxy> micStateChangeListenerCb = it->second;
    ...
    const uint32_t sessionID, const uint32_t code)
{
    ...
    if (!audioPolicyService_.IsSessionIdValid(callerUid, sessionID)) {
        AUDIO_ERR_LOG("RegisterAudioInterruptCallbackClient for sessionID %{public}d, id is invalid", sessionID);
        return ERR_INVALID_PARAM;
    }

    CHECK_AND_RETURN_RET_LOG(object != nullptr, ERR_INVALID_PARAM, \
        "RegisterAudioInterruptCallbackClient object is nullptr");
    std::shared_ptr<AudioPolicyClientProxy> proxy = GetAudioPolicyClientProxy(sessionID, object);
    if (proxy == nullptr) {
        AUDIO_ERR_LOG("AudioPolicyServer::RegisterAudioInterruptCallbackClient, proxy is null.");
...
int32_t AudioPolicyServer::UnRegisterAudioInterruptCallbackClient(const uint32_t sessionID, const uint32_t code)
{
    ...
    if (proxy == nullptr) {
        AUDIO_ERR_LOG("AudioPolicyServer::UnRegisterAudioInterruptCallbackClient, proxy is null.");
        return ERR_INVALID_OPERATION;
    }
    proxy->UnregisterPolicyCallbackClient(code);

    std::lock_guard<std::mutex> lock(interruptMutex_);
    if (audioPolicyClientProxyCBMap_.erase(sessionID) == 0) {
...
void AudioPolicyServer::ProcessCurrentInterrupt(const AudioInterrupt &incomingInterrupt)
{
    ...
    for (auto iterActive = audioFocusInfoList_.begin(); iterActive != audioFocusInfoList_.end();) {
        ...
        std::shared_ptr<AudioPolicyClientProxy> policyListenerCb = audioPolicyClientProxyCBMap_[activeSessionID];
    ...
    std::list<std::pair<AudioInterrupt, AudioFocuState>>::iterator &iterActive)
{
    ...
    std::shared_ptr<AudioPolicyClientProxy> policyListenerCb = audioPolicyClientProxyCBMap_[sessionID];
...
void AudioPolicyServer::OnAudioFocusInfoChange()
{
    ...
    for (auto it = audioPolicyClientProxyCBMap_.begin(); it != audioPolicyClientProxyCBMap_.end(); ++it) {
        std::shared_ptr<AudioPolicyClientProxy> foucusInfoCb = it->second;
...
void AudioPolicyServer::ProcessInterrupt(const InterruptHint& hint)
{
    ...
    for (auto it : audioPolicyClientProxyCBMap_) {
...
void AudioPolicyServer::RegisteredStreamListenerClientDied(pid_t pid)
{
    ...
    audioPolicyService_.RegisteredStreamListenerClientDied(pid,
        static_cast<int32_t>(AudioPolicyClientCode::ON_RENDERERSTATE_CHANGE), hasBTPermission, hasSysPermission);
    audioPolicyService_.RegisteredStreamListenerClientDied(pid,
        static_cast<int32_t>(AudioPolicyClientCode::ON_CAPTURERSTATE_CHANGE), hasBTPermission, hasSysPermission);
}
    ...
    const std::string& condition)
{
    ...
    InterruptEventInternal interruptEvent {type, forceType, hint, 0.2f};
    for (auto it : server_->audioPolicyClientProxyCBMap_) {
        if (it.second != nullptr) {
            it.second->OnInterrupt(interruptEvent);
        }
    }
...
int32_t AudioPolicyServer::RegisterPolicyCallbackClient(const sptr<IRemoteObject> &object, const int32_t code)
{
    int32_t clientPid = IPCSkeleton::GetCallingPid();
    ...
    } else if ((code == static_cast<int32_t>(AudioPolicyClientCode::ON_RENDERERSTATE_CHANGE)) ||
        (code == static_cast<int32_t>(AudioPolicyClientCode::ON_CAPTURERSTATE_CHANGE))) {
        bool hasBTPermission = VerifyPermission(USE_BLUETOOTH_PERMISSION);
        bool hasSysPermission = PermissionUtil::VerifySystemPermission();
        return audioPolicyService_.RegisterRendererOrCapturerEventListenerCbClient(object, clientPid, code,
            hasBTPermission, hasSysPermission);
    } else {
...
int32_t AudioPolicyServer::UnregisterPolicyCallbackClient(const int32_t code)
{
    int32_t clientPid = IPCSkeleton::GetCallingPid();
    if ((code == static_cast<int32_t>(AudioPolicyClientCode::ON_DEVICE_CHANGE)) ||
        (code == static_cast<int32_t>(AudioPolicyClientCode::ON_ACTIVE_OUTPUT_DEVICE_UPDATED)) ||
        ...
        bool hasBTPermission = VerifyPermission(USE_BLUETOOTH_PERMISSION);
        bool hasSysPermission = PermissionUtil::VerifySystemPermission();
        return audioPolicyService_.UnregisterRendererOrCapturerEventListenerCbClient(clientPid, code, hasBTPermission,
            hasSysPermission);
    } else {
