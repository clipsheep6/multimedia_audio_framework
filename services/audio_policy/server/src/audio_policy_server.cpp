    ...
    const AudioStreamType &streamInFocus)
{
    ...
    if (volLevelCheck) {
        for (auto it = audioPolicyClientProxyCBMap_.begin(); it != audioPolicyClientProxyCBMap_.end(); ++it) {
            sptr<AudioPolicyClientProxy> volumeChangeCb = it->second;
...
sptr<AudioPolicyClientProxy> AudioPolicyServer::GetAudioPolicyClientProxy(
    const int32_t clientPid, bool hasBTPermission, bool hasSystemPermission, const sptr<IRemoteObject> &object)
{
    sptr<AudioPolicyClientProxy> proxy = audioPolicyClientProxyCBMap_[clientPid];
    if (proxy == nullptr && object != nullptr) {
        proxy = new(std::nothrow) AudioPolicyClientProxy(object);
        proxy->hasBTPermission_ = hasBTPermission;
        proxy->hasSystemPermission_ = hasSystemPermission;
        audioPolicyClientProxyCBMap_[clientPid] = proxy;
    }
    return proxy;
}
...
int32_t AudioPolicyServer::SetRingerMode(AudioRingerMode ringMode, API_VERSION api_v)
{
    ...
    if (ret == SUCCESS) {
        for (auto it = audioPolicyClientProxyCBMap_.begin(); it != audioPolicyClientProxyCBMap_.end(); ++it) {
            sptr<AudioPolicyClientProxy> ringerModeListenerCb = it->second;
...
int32_t AudioPolicyServer::SetMicrophoneMuteCommon(bool isMute, API_VERSION api_v)
{
    ...
    if (ret == SUCCESS && isMicrophoneMute != isMute) {
        for (auto it = audioPolicyClientProxyCBMap_.begin(); it != audioPolicyClientProxyCBMap_.end(); ++it) {
            sptr<AudioPolicyClientProxy> micStateChangeListenerCb = it->second;
...
void AudioPolicyServer::OnAudioFocusInfoChange()
{
    ...
    for (auto it = audioPolicyClientProxyCBMap_.begin(); it != audioPolicyClientProxyCBMap_.end(); ++it) {
        sptr<AudioPolicyClientProxy> foucusInfoCb = it->second;
...
int32_t AudioPolicyServer::RegisterPolicyCallbackClient(const sptr<IRemoteObject> &object)
{
    ...
    // } else {
    //     sptr<AudioPolicyClientProxy> proxy = GetAudioPolicyClientProxy(clientPid, object);
    //     if (proxy == nullptr) {
    //         return ERR_INVALID_OPERATION;
    //     }
    ...
    sptr<AudioPolicyClientProxy> proxy = GetAudioPolicyClientProxy(clientPid, hasBTPermission,
        hasSysPermission, object);
    if (proxy == nullptr) {
        return ERR_INVALID_OPERATION;
    }
    return proxy->RegisterPolicyCallbackClient(object);
}
...
int32_t AudioPolicyServer::UnregisterPolicyCallbackClient()
{
    ...
    // } else {
        // sptr<AudioPolicyClientProxy> proxy = GetAudioPolicyClientProxy(clientPid, nullptr);
    int32_t clientPid = IPCSkeleton::GetCallingPid();
    bool hasBTPermission = VerifyPermission(USE_BLUETOOTH_PERMISSION);
    bool hasSysPermission = PermissionUtil::VerifySystemPermission();
    sptr<AudioPolicyClientProxy> proxy = GetAudioPolicyClientProxy(clientPid, hasBTPermission,
        hasSysPermission, nullptr);
    if (proxy == nullptr) {
        return ERR_INVALID_OPERATION;
    }
    proxy->UnregisterPolicyCallbackClient();
