...
{
    std::vector<std::unique_ptr<AudioDeviceDescriptor>> descs = mPolicyService.GetAvailableDevices(usage);
    return descs;
}
        ...
        const sptr<IRemoteObject> &object)
{
    ...
    int32_t clientPid = IPCSkeleton::GetCallingPid();
    bool hasBTPermission = VerifyPermission(USE_BLUETOOTH_PERMISSION);
    return mPolicyService.SetAvailableDeviceChangeCallback(clientPid, usage, object, hasBTPermission);
}

int32_t AudioPolicyServer::UnSetAvailableDeviceChangeCallback(const int32_t /*clientId*/, AudioDeviceUsage usage)
{
    int32_t clientPid = IPCSkeleton::GetCallingPid();
    return mPolicyService.UnSetAvailableDeviceChangeCallback(clientPid, usage);
}
} // namespace AudioStandard
} // namespace OHOS
