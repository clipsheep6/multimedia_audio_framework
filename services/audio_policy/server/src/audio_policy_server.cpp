...
int32_t AudioPolicyServer::SetWakeUpAudioCapturer(InternalAudioCapturerOptions options)
{
    ...
    CHECK_AND_RETURN_RET_LOG(hasManageIntellgentPermission, ERR_PERMISSION_DENIED,
        "No permission");
...
int32_t AudioPolicyServer::VerifyVoiceCallPermission(uint64_t fullTokenId, Security::AccessToken::AccessTokenID tokenId)
{
    ...
    CHECK_AND_RETURN_RET_LOG(hasRecordVoiceCallPermission, ERR_PERMISSION_DENIED,
        "No permission");
    return SUCCESS;
}
...
int32_t AudioPolicyServer::UnsetAudioInterruptCallback(const uint32_t sessionID)
{
    ...
    CHECK_AND_RETURN_RET_LOG(interruptCbsMap_.erase(sessionID) != 0, ERR_INVALID_OPERATION,
        "session %{public}d not present", sessionID);
...
int32_t AudioPolicyServer::UnsetAudioManagerInterruptCallback(const int32_t /* clientId */)
{
    std::lock_guard<std::mutex> lock(amInterruptMutex_);
    int32_t clientPid = IPCSkeleton::GetCallingPid();
    CHECK_AND_RETURN_RET_LOG(amInterruptCbsMap_.erase(clientPid) != 0, ERR_INVALID_OPERATION,
        "client %{public}d not present", clientPid);
...
void AudioPolicyServer::NotifyFocusGranted(const int32_t clientId, const AudioInterrupt &audioInterrupt)
{
    ...
    std::lock_guard<std::mutex> lock(amInterruptMutex_);
    CHECK_AND_RETURN_LOG(amInterruptCbsMap_.find(clientId) != amInterruptCbsMap_.end(),
        "Notify focus granted in: %{public}d failed, callback does not exist", clientId);
...
int32_t AudioPolicyServer::NotifyFocusAbandoned(const int32_t clientId, const AudioInterrupt &audioInterrupt)
{
    ...
    CHECK_AND_RETURN_RET_LOG(interruptCb, ERR_INVALID_PARAM,
        "Notify failed, callback not present");
    ...
    const std::string& condition)
{
    ...
    for (auto it = server_->audioPolicyClientProxyCBMap_.begin(); it != server_->audioPolicyClientProxyCBMap_.end();
        ...
        CHECK_AND_CONTINUE_LOG(volumeChangeCb != nullptr, "volumeChangeCb: nullptr for client : %{public}d", it->first);
