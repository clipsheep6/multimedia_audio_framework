...
bool audioAffinityParser::Parse()
{
    ...
    audioAffinityManager_->OnAffinityXmlParsingCompleted(affinityDeviceInfoArray_);
    return true;
}
...
void audioAffinityParser::ParserAffinityGroupDeviceInfos(xmlNode *node, AffinityDeviceInfo& deviceInfo)
{
    ...
    while (currNode) {
        if (XML_ELEMENT_NODE == currNode->type &&
            ...
            std::map<std::string, DeviceType>::iterator item = deviceType_.find(std::string(pValue));
            deviceInfo.deviceType_ = (item != deviceType_.end() ? item->second : DEVICE_TYPE_INVALID);
            xmlFree(pValue);
