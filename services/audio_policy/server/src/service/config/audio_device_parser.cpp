...
void AudioDeviceParser::ParseDeviceCategory(const std::string deviceCategory, int32_t &deviceCategoryFlag)
{
    ...
    for (itr i = buf.begin(); i != buf.end(); i++) {
        if (*i == "HEADPHONE") {
            deviceCategoryFlag |= DeviceCategory::HEADPHONE;
        } else if (*i == "GLASSES") {
            deviceCategoryFlag |= DeviceCategory::GLASSES;
        } else if (*i == "SOUNDBOX") {
            deviceCategoryFlag |= DeviceCategory::SOUNDBOX;
        } else if (*i == "CAR") {
            deviceCategoryFlag |= DeviceCategory::CAR;
        } else if (*i == "HEADPHONE_UNWEAR") {
            deviceCategoryFlag |= DeviceCategory::HEADPHONE_UNWEAR;
        } else if (*i == "WATCH") {
            deviceCategoryFlag |= DeviceCategory::WATCH;
        }
    }
}
