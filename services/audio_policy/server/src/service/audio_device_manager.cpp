...
void AudioDeviceManager::OnXmlParsingCompleted(
    const std::unordered_map<AudioDevicePrivacyType, std::list<DevicePrivacyInfo>> &xmlData)
{
    if (xmlData.empty()) {
    ...
    DeviceUsage devUsage, string logName)
{
    ...
    if (result) {
        ...
        AUDIO_ERR_LOG("Add to %{public}s list.", logName.c_str());
    ...
    const AudioDeviceDescriptor &devDesc, string logName)
{
    ...
    auto itr = std::find_if(descArray.begin(), descArray.end(), isPresent);
