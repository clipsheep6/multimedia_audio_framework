...
static int32_t ParseEffectConfigFile(xmlDoc* &doc)
{
    ...
    for (int32_t i = MAX_CFG_POLICY_DIRS_CNT - 1; i >= 0; i--) {
        if (cfgFiles->paths[i] && *(cfgFiles->paths[i]) != '\0') {
            ...
            doc = xmlReadFile(cfgFiles->paths[i], nullptr, static_cast<int32_t>(
                static_cast<uint32_t>(XML_PARSE_NOERROR) | static_cast<uint32_t>(XML_PARSE_NOWARNING)));
            break;
        }
    }
    ...
    doc = xmlReadFile(AUDIO_EFFECT_CONFIG_FILE, nullptr,
        static_cast<int32_t>(static_cast<uint32_t>(XML_PARSE_NOERROR) | static_cast<uint32_t>(XML_PARSE_NOWARNING)));
#endif
    if (doc == nullptr) {
        std::shared_ptr<Media::MediaMonitor::EventBean> bean = std::make_shared<Media::MediaMonitor::EventBean>(
            Media::MediaMonitor::AUDIO, Media::MediaMonitor::LOAD_CONFIG_ERROR,
