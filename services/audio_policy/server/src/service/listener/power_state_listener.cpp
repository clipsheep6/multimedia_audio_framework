...
const int32_t AUDIO_RELEASE_LOCK_DELAY_S = 5;

int32_t PowerStateCallbackStub::OnRemoteRequest(uint32_t code, MessageParcel &data,
    MessageParcel &reply, MessageOption &option)
{
    AUDIO_DEBUG_LOG("code = %{public}d, flag = %{public}d", code, option.GetFlags());
    std::u16string descriptor = PowerStateCallbackStub::GetDescriptor();
...
int32_t PowerStateCallbackStub::OnSyncSleepCallbackStub(MessageParcel &data)
{
...
int32_t PowerStateCallbackStub::OnSyncWakeupCallbackStub(MessageParcel &data)
{
    bool forceSleep = data.ReadBool();
...
void PowerStateListener::ControlAudioFocus(bool applyFocus)
{
    ...
    audioInterrupt.streamUsage = StreamUsage::STREAM_USAGE_VOICE_COMMUNICATION;
    audioInterrupt.audioFocusType.streamType = AudioStreamType::STREAM_VOICE_CALL;
    ...
    if (applyFocus) {
        ...
        if (result != SUCCESS) {
            ...
            // Waiting for the audio policy service to release the lock
            std::this_thread::sleep_for(std::chrono::seconds(AUDIO_RELEASE_LOCK_DELAY_S));
        }
    } else {
