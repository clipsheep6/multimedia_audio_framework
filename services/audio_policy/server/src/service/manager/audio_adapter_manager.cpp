...
bool AudioAdapterManager::Init()
{
    ...
    if (ret > 0) {
        ...
        AUDIO_WARNING_LOG("Init: Get volume parameter failed %{public}d", ret);
    }
...
void AudioAdapterManager::SaveMediaVolumeToLocal(AudioStreamType streamType, int32_t volumeLevel)
{
    if (streamType == STREAM_MUSIC) {
        ...
        if (ret == 0) {
            ...
            AUDIO_WARNING_LOG("Save media volume for boot failed, result %{public}d", ret);
        }
    }
}
...
void AudioAdapterManager::WriteVolumeToKvStore(DeviceType type, AudioStreamType streamType, int32_t volumeLevel)
{
    CHECK_AND_RETURN_LOG(audioPolicyKvStore_ != nullptr,
        "WriteVolumeToKvStore failed: audioPolicyKvStore_ is nullptr");
    ...
    if (status == Status::SUCCESS) {
        ...
        AUDIO_WARNING_LOG("WriteVolumeToKvStore: Failed to write volumeLevel %{public}d for %{public}s to kvStore!",
