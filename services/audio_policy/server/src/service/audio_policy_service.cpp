...
bool AudioPolicyService::IsIncomingDeviceInRemoteRender(sptr<AudioDeviceDescriptor> incomingDevice)
{
...
bool AudioPolicyService::IsIncomingDeviceInRemoteCapture(sptr<AudioDeviceDescriptor> incomingDevice)
{
    vector<unique_ptr<AudioDeviceDescriptor>> descriptors = audioDeviceManager_.GetRemoteCaptureDevices();
    for (const auto &desc : descriptors) {
        if (desc != nullptr) {
            if (desc->deviceRole_ == incomingDevice->deviceRole_ &&
                desc->deviceType_ == incomingDevice->deviceType_ &&
