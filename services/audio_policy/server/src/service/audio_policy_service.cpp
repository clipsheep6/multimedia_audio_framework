...
bool AudioPolicyService::Init(void)
{
    ...
    CHECK_AND_RETURN_RET_LOG(!audioToneParser->LoadConfig(toneDescriptorMap), false,
        "Audio Tone Load Configuration failed");
#endif
    ...
    CHECK_AND_RETURN_RET_LOG(!deviceStatusListener_->RegisterDeviceStatusListener(), false,
        "[Policy Service] Register for device status events failed");
...
void AudioPolicyService::SetOffloadMode()
{
    ...
    AUDIO_DEBUG_LOG("sessionId: %{public}d, PowerState: %{public}d, isAppBack: %{public}d",
...
std::string AudioPolicyService::GetSelectedDeviceInfo(int32_t uid, int32_t pid, AudioStreamType streamType)
{
    ...
    if (!routerMap_.count(uid)) {
        AUDIO_DEBUG_LOG("GetSelectedDeviceInfo no such uid[%{public}d]", uid);
        return "";
    }
    ...
    if (routerMap_[uid].second == pid) {
        ...
        AUDIO_DEBUG_LOG("GetSelectedDeviceInfo: uid[%{public}d] changed pid, get local as defalut", uid);
        routerMap_.erase(uid);
        selectedDevice = LOCAL_NETWORK_ID;
    }

    if (LOCAL_NETWORK_ID == selectedDevice) {
        AUDIO_DEBUG_LOG("GetSelectedDeviceInfo: uid[%{public}d]-->local.", uid);
        return "";
    }
    // check if connected.
    ...
    if (isConnected) {
        AUDIO_DEBUG_LOG("GetSelectedDeviceInfo result[%{public}s]", selectedDevice.c_str());
        return selectedDevice;
    } else {
        routerMap_.erase(uid);
        AUDIO_DEBUG_LOG("GetSelectedDeviceInfo device already disconnected.");
        return "";
    }
}
...
{
    AUDIO_DEBUG_LOG("SelectOutputDevice start for uid[%{public}d]", audioRendererFilter->uid);
...
{
    AUDIO_DEBUG_LOG("SelectFastOutputDevice for uid[%{public}d] device[%{public}s]", audioRendererFilter->uid,
...
int32_t AudioPolicyService::RememberRoutingInfo(sptr<AudioRendererFilter> audioRendererFilter,
    sptr<AudioDeviceDescriptor> deviceDescriptor)
{
    AUDIO_DEBUG_LOG("RememberRoutingInfo for uid[%{public}d] device[%{public}s]", audioRendererFilter->uid,
        deviceDescriptor->networkId_.c_str());
...
{
    AUDIO_DEBUG_LOG("MoveToLocalOutputDevice for [%{public}zu] sink-inputs", sinkInputIds.size());
    // check
...
int32_t AudioPolicyService::MoveToRemoteOutputDevice(std::vector<SinkInput> sinkInputIds,
    sptr<AudioDeviceDescriptor> remoteDeviceDescriptor)
{
    AUDIO_DEBUG_LOG("MoveToRemoteOutputDevice for [%{public}zu] sink-inputs", sinkInputIds.size());
    ...
    sptr<AudioDeviceDescriptor> deviceDescriptor)
{
    ...
    AUDIO_DEBUG_LOG("SelectFastInputDevice for uid[%{public}d] device[%{public}s]", audioCapturerFilter->uid,
        deviceDescriptor->networkId_.c_str());
    return SUCCESS;
}
...
int32_t AudioPolicyService::SelectInputDevice(sptr<AudioCapturerFilter> audioCapturerFilter,
    std::vector<sptr<AudioDeviceDescriptor>> audioDeviceDescriptors)
{
    AUDIO_DEBUG_LOG("Select input device start for uid[%{public}d]", audioCapturerFilter->uid);
...
int32_t AudioPolicyService::MoveToRemoteInputDevice(std::vector<SourceOutput> sourceOutputs,
    sptr<AudioDeviceDescriptor> remoteDeviceDescriptor)
{
    AUDIO_DEBUG_LOG("MoveToRemoteInputDevice start");
...
void AudioPolicyService::OnPreferredOutputDeviceUpdated(const AudioDeviceDescriptor& deviceDescriptor)
{
    ...
    AUDIO_DEBUG_LOG("OnPreferredOutputDeviceUpdated start");
...
{
    AUDIO_DEBUG_LOG("OnPreferredInputDeviceUpdated start");
...
{
    AUDIO_DEBUG_LOG("SetWakeUpAudioCapturer start");
...
{
    AUDIO_DEBUG_LOG("CloseWakeUpAudioCapturer start");
...
{
    AUDIO_DEBUG_LOG("fetch output device when no running stream");
...
{
    AUDIO_DEBUG_LOG("fetch input device when no running stream");
...
{
    AUDIO_DEBUG_LOG("Fetch output device for %{public}zu stream", rendererChangeInfos.size());
...
void AudioPolicyService::FetchInputDevice(vector<unique_ptr<AudioCapturerChangeInfo>> &capturerChangeInfos,
    bool isStreamStatusUpdated = false)
{
    AUDIO_DEBUG_LOG("Fetch input device for %{public}zu stream", capturerChangeInfos.size());
    bool needUpdateActiveDevice = true;
    bool isUpdateActiveDevice = false;
...
{
    AUDIO_DEBUG_LOG("IsSessionIdValid::callerUid: %{public}d, sessionId: %{public}d", callerUid, sessionId);
...
{
    AUDIO_DEBUG_LOG("ReloadA2dpAudioPort: switch device from a2dp to another a2dp, reload a2dp module");
...
{
    AUDIO_DEBUG_LOG("HandleFileDevice start");
...
{
    AUDIO_DEBUG_LOG("Switch device: [%{public}d]-->[%{public}d]", currentActiveDevice_.deviceType_, deviceType);
    int32_t result = SUCCESS;
    ResetOffloadMode();
...
{
    AUDIO_DEBUG_LOG("SetDeviceActive: Device type[%{public}d] flag[%{public}d]", deviceType, active);
...
{
    AUDIO_DEBUG_LOG("SetAudioScene: %{public}d", audioScene);
    const sptr<IStandardAudioService> gsp = GetAudioServerProxy();
    CHECK_AND_RETURN_RET_LOG(gsp != nullptr, ERR_OPERATION_FAILED, "Service proxy unavailable");
...
void AudioPolicyService::UpdateConnectedDevicesWhenConnectingForOutputDevice(
    ...
    sptr<AudioDeviceDescriptor> &audioDescriptor)
{
    AUDIO_DEBUG_LOG("Filling output device for %{public}d", deviceDescriptor.deviceType_);
...
void AudioPolicyService::UpdateConnectedDevicesWhenConnectingForInputDevice(
    ...
    sptr<AudioDeviceDescriptor> &audioDescriptor)
{
    AUDIO_DEBUG_LOG("Filling input device for %{public}d", deviceDescriptor.deviceType_);
...
{
    AUDIO_DEBUG_LOG("macAddress:[%{public}s]", macAddress.c_str());
    {
        std::lock_guard<std::mutex> lock(a2dpDeviceMapMutex_);
...
void AudioPolicyService::OnDeviceConfigurationChanged(DeviceType deviceType, const std::string &macAddress,
    const std::string &deviceName, const AudioStreamInfo &streamInfo)
{
    AUDIO_DEBUG_LOG("OnDeviceConfigurationChanged start");
...
{
    AUDIO_DEBUG_LOG("RegisterRemoteDevStatusCallback start");
...
{
    AUDIO_DEBUG_LOG("Device connection updated | HDI_PIN[%{public}d] CONNECT_STATUS[%{public}d] NETWORKID[%{public}s]",
...
{
    AUDIO_DEBUG_LOG("[module_load]::OnServiceConnected for [%{public}d]", serviceIndex);
    ...
    if (serviceFlag_.count() != MIN_SERVICE_COUNT) {
        AUDIO_DEBUG_LOG("[module_load]::hdi service or audio service not up. Cannot load default module now");
        return;
    }
    ...
    for (const auto &device : deviceClassInfo_) {
        if (device.first == ClassType::TYPE_PRIMARY || device.first == ClassType::TYPE_FILE_IO) {
            ...
            for (auto &moduleInfo : moduleInfoList) {
                AUDIO_DEBUG_LOG("[module_load]::Load module[%{public}s]", moduleInfo.name.c_str());
    ...
    if (result == SUCCESS) {
        AUDIO_DEBUG_LOG("[module_load]::Setting speaker as active device on bootup");
...
{
    AUDIO_DEBUG_LOG("AudioPolicyService::OnMonoAudioConfigChanged: audioMono = %{public}s", audioMono? "true": "false");
    const sptr<IStandardAudioService> gsp = GetAudioServerProxy();
...
{
    AUDIO_DEBUG_LOG("AudioPolicyService::OnAudioBalanceChanged: audioBalance = %{public}f", audioBalance);
    const sptr<IStandardAudioService> gsp = GetAudioServerProxy();
    CHECK_AND_RETURN_LOG(gsp != nullptr, "Service proxy unavailable: g_adProxy null");
...
{
    AUDIO_DEBUG_LOG("LoadSinksForCapturer start");
...
{
    AUDIO_DEBUG_LOG("LoadInnerCapturerSink start");
...
{
    AUDIO_DEBUG_LOG("LoadReceiverSink start");
    AudioModuleInfo moduleInfo = {};
...
void AudioPolicyService::LoadLoopback()
{
    ...
    AUDIO_DEBUG_LOG("LoadLoopback start");
...
{
    std::string module;
    AUDIO_DEBUG_LOG("UnloadLoopback start");
...
{
    AUDIO_DEBUG_LOG("device class num [%{public}zu]", xmlData.size());
...
{
    AUDIO_DEBUG_LOG("group data num [%{public}zu]", volumeGroupData.size());
...
{
    AUDIO_DEBUG_LOG("group data num [%{public}zu]", interruptGroupData.size());
...
{
    AUDIO_DEBUG_LOG("UnsetAvailableDeviceChangeCallback start");
...
{
    AUDIO_DEBUG_LOG("UpdateTrackerDeviceChange start");
...
int32_t AudioPolicyService::SetDeviceAbsVolumeSupported(const std::string &macAddress, const bool support)
{
    ...
    AUDIO_DEBUG_LOG("SetDeviceAbsVolumeSupported success for macAddress:[%{public}s], support: %{public}d",
...
{
    AUDIO_DEBUG_LOG("RegiestPolicy start");
    const sptr<IStandardAudioService> gsp = GetAudioServerProxy();
...
{
    AUDIO_DEBUG_LOG("InitSharedVolume start");
    CHECK_AND_RETURN_RET_LOG(policyVolumeMap_ != nullptr && policyVolumeMap_->GetBase() != nullptr,
...
{
    AUDIO_DEBUG_LOG("SetParameterCallback start");
...
void AudioPolicyService::RegisterDataObserver()
{
    ...
    std::string devicesName = "";
    int32_t ret = GetDeviceNameFromDataShareHelper(devicesName);
    AUDIO_DEBUG_LOG(":UpdateDisplayName local name [%{public}s]", devicesName.c_str());
    CHECK_AND_RETURN_LOG(ret == SUCCESS, "Local UpdateDisplayName init device failed");
...
void AudioPolicyService::OnCapturerSessionRemoved(uint64_t sessionID)
{
    ...
    AUDIO_DEBUG_LOG("Sessionid:%{public}" PRIu64 " not added, directly placed into sessionIdisRemovedSet_", sessionID);
...
void AudioPolicyService::OnCapturerSessionAdded(uint64_t sessionID, SessionInfo sessionInfo)
{
    ...
    AUDIO_DEBUG_LOG("sessionID: %{public}" PRIu64 " OnCapturerSessionAdded end", sessionID);
}
...
{
    AUDIO_DEBUG_LOG("[OnScoStateChanged] macAddress: %{public}s, isConnnected: %{public}d", macAddress.c_str(),
        isConnnected);
...
{
    AUDIO_DEBUG_LOG("[OnDeviceInfoUpdated]  updateCommand: %{public}d", updateCommand);
