...
std::tuple<SourceType, uint32_t, uint32_t> AudioPolicyService::FetchTargetInfoForSessionAdd(const SessionInfo sessionInfo)
{
    ...
    if (sessionInfo.sourceType == SOURCE_TYPE_VOICE_COMMUNICATION) {
        ...
        if (primaryMicModuleInfo_.supportedChannels_.count(targetChannels) == 0) {
            AUDIO_INFO_LOG("targetChannels: %{public}u is not supported rate, using highestSupportedChannels: %{public}u",
