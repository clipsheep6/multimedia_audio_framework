    ...
    vector<std::unique_ptr<AudioDeviceDescriptor>> &outputDevices, const AudioStreamDeviceChangeReason reason)
{
    ...
    if (isUpdateRouteSupported_) {
        if (IsRingerOrAlarmerStreamUsage(streamUsage) && IsRingerOrAlarmerDualDevicesRange(deviceType)) {
            if (!DealRingerOrAlarmerDualDevices(outputDevices)) {
            ...
            if (ringerMode != RINGER_MODE_NORMAL && IsRingerOrAlarmerDualDevicesRange(descs.front()->getType()) &&
                ...
                if (audioPolicyManager_.SetStreamMute(STREAM_RING, false) == SUCCESS) {
    ...
    const AudioStreamDeviceChangeReason reason)
{
    ...
    for (auto &rendererChangeInfo : rendererChangeInfos) {
        ...
        if (HandleDeviceChangeForFetchOutputDevice(descs.front(), rendererChangeInfo) == ERR_NEED_NOT_SWITCH_DEVICE &&
            streamUsage != STREAM_USAGE_ALARM  && streamUsage != STREAM_USAGE_RINGTONE &&
...
bool AudioPolicyService::DealRingerOrAlarmerDualDevices(vector<std::unique_ptr<AudioDeviceDescriptor>> &descs)
{
    CHECK_AND_RETURN_RET_LOG(descs.size() > 0 && descs.size() <= AUDIO_ACTIVE_DEVICES_LIMIT, false,
...
int32_t AudioPolicyService::SetRingerStreamMute()
{
    if (!ringerModeMute_) {
...
bool AudioPolicyService::GetRingerModeMute()
{
