...
const int32_t HOUR_TO_MINUTE = 60;
const int32_t TIMEOUT_TWENTY_HOUR = 20;
...
void AudioPolicyService::CheckBlueToothActiveMusicTime(int32_t safeVolume)
{
    ...
    if (activeSafeTimeBt_ >= ONE_MINUTE * HOUR_TO_MINUTE * TIMEOUT_TWENTY_HOUR) {
        AUDIO_INFO_LOG("CheckBlueToothActiveMusicTime timeout");
...
void AudioPolicyService::CheckWiredActiveMusicTime(int32_t safeVolume)
{
    ...
    int32_t currentTime = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
    if (activeSafeTime_ >= ONE_MINUTE * HOUR_TO_MINUTE * TIMEOUT_TWENTY_HOUR) {
        AUDIO_INFO_LOG("CheckWiredActiveMusicTime timeout");
...
{
    AUDIO_INFO_LOG("enter CheckActiveMusicTime");
...
{
    auto ams = AAFwk::AbilityManagerClient::GetInstance();
    sptr<OHOS::AAFwk::IAbilityConnection> dialogConnectionCallback = new AudioDialogAbilityConnection();
    CHECK_AND_RETURN_RET_LOG(dialogConnectionCallback != nullptr, ERROR, "ams malloc failed");
    CHECK_AND_RETURN_RET_LOG(ams != nullptr, ERROR, "ams malloc failed");

    AAFwk::Want want;
    std::string bundleName = "com.ohos.sceneboard";
    std::string abilityName = "com.ohos.sceneboard.systemdialog";
    want.SetElementName(bundleName, abilityName);
    ErrCode result = ams->ConnectAbility(want, dialogConnectionCallback, AppExecFwk::Constants::INVALID_USERID);
    if (result != SUCCESS) {
        bundleName = "com.ohos.systemui";
        abilityName = "com.ohos.systemui.dialog";
        want.SetElementName(bundleName, abilityName);
        result = ams->ConnectAbility(want, dialogConnectionCallback, AppExecFwk::Constants::INVALID_USERID);
    }
...
{
    AUDIO_INFO_LOG("DisableSafeMediaVolume");
