...
std::unique_ptr<AudioDeviceDescriptor> AudioAffinityManager::GetCaptureDevice(int32_t clientUID)
{
    std::lock_guard<std::mutex> lock(capturerMapMutex_);
    int32_t affinityClientUID = clientUID;
    AFFINITYDEVMAP::iterator item = activeCaptureDeviceMap_.find(clientUID);
    if (item != activeCaptureDeviceMap_.end()) {
        if (item->second == nullptr) {
            AUDIO_INFO_LOG("AudioDeviceDescriptor sptr not valid");
            return make_unique<AudioDeviceDescriptor>();
        }
        unique_ptr<AudioDeviceDescriptor> desc =
            make_unique<AudioDeviceDescriptor>(activeCaptureDeviceMap_[affinityClientUID]);
        return desc;
    } else {
        return make_unique<AudioDeviceDescriptor>();
    }
...
void AudioAffinityManager::AddSelectRendererDevice(int32_t clientUID, const sptr<AudioDeviceDescriptor> &desc)
{
    ...
    std::unordered_map<int32_t, AffinityDeviceInfo> affinityDeviceInfoMap =
        GetActiveAffinityDeviceMapByGroupName(activeRendererDeviceMap_, affinityDeviceInfo.groupName);
...
void AudioAffinityManager::AddSelectCaptureDevice(int32_t clientUID, const sptr<AudioDeviceDescriptor> &desc)
{
    std::lock_guard<std::mutex> lock(capturerMapMutex_);
    CHECK_AND_RETURN_LOG(desc != nullptr, "AudioDeviceDescriptor sptr not valid");
    ...
    std::unordered_map<int32_t, AffinityDeviceInfo> affinityDeviceInfoMap =
        GetActiveAffinityDeviceMapByGroupName(activeCapturerDeviceMap_, affinityDeviceInfo.groupName);
...
void AudioAffinityManager::DelSelectRendererDevice(int32_t clientUID)
{
    ...
    if (item != activeRendererDeviceMap_.end()) {
        ...
        activeRendererDeviceMap_.earse(item);
    }
}
...
void AudioAffinityManager::DelSelectCapturerDevice(int32_t clientUID)
{
    ...
    if (item != activeCaptureDeviceMap_.end()) {
        ...
        activeCaptureDeviceMap_.earse(item);
    }
}
...
void AudioAffinityManager::RemoveOfflineRendererDevice(const AudioDeviceDescriptor &updateDesc)
{
    ...
    while (item != activeRendererDeviceMap_.end()) {
        CHECK_AND_RETURN_LOG(item->second != nullptr, "AudioDeviceDescriptor sptr not valid");
        if (item->second->deviceType_ == updateDesc.deviceType &&
...
{
    std::lock_guard<std::mutex> lock(capturerMapMutex_);
    AFFINITYDEVMAP::iterator item = activeCaptureDeviceMap_.begin();
    while (item != activeCaptureDeviceMap_.end()) {
        CHECK_AND_RETURN_LOG(item->second != nullptr, "AudioDeviceDescriptor sptr not valid");
        if (item->second->deviceType_ == updateDesc.deviceType &&
            item->second->networkId_ == updateDesc.networkId_) {
            DelActiveGroupAffinityMap(item->first, item->second->getType(), item->second->netWorkId_,
...
{
    AffinityDeviceInfo affinityDeviceInfo = {};
    std::vector<AffinityDeviceInfo>::const_iterctor item = affinityDeviceInfo.begin();
...
{
    std::unordered_map<int32_t, AffinityDeviceInfo> affinityDeviceInfoMap = {};
    AFFINITYDEVINFOMAP::const_iterctor itemGroup = avtiveGroupNameMap.find(groupName);
    ...
    std::unordered_map<int32_t, AffinityDeviceInfo> &affinityDeviceInfoMap)
{
    ...
    if (item != affinityDeviceInfoMap.end()) {
        affinityDeviceInfoMap.earse(item);
    }
}

void AudioAffinityManager::DelActiveGroupAffinityMap(const int32_t &clientUID, const DeviceType &deviceType,
    const std::string &networkID, std::vector<AffinityDeviceInfo> &affinityDeviceInfoArray,
    AFFINITYDEVINFOMAP &activeGroupNameMap)
{
    AffinityDeviceInfo affinityDeviceInfo =
        GetAffinityDeviceInfoByDeviceType(affinityDeviceInfoArray, deviceType, networkID);
