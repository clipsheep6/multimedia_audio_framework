...
{
    AUDIO_DEBUG_LOG("~AudioConcurrencyStateListenerProxy: Instance destroy");
}
...
void AudioConcurrencyStateListenerProxy::OnConcedeStream()
{
    ...
    if (!data.WriteInterfaceToken(GetDescriptor())) {
        AUDIO_ERR_LOG("lxj AudioConcurrencyStateListenerProxy: WriteInterfaceToken failed");
        return;
    }

    int error = Remote()->SendRequest(ON_CONCEDE_STREAM, data, reply, option);
    if (error != ERR_NONE) {
        AUDIO_ERR_LOG("lxj OnConcedeStream failed, error: %{public}d", error);
    }
}
...
{
        AUDIO_DEBUG_LOG("AudioConcurrencyListenerCallback: Instance create");
}

AudioConcurrencyListenerCallback::~AudioConcurrencyListenerCallback()
{
    AUDIO_DEBUG_LOG("AudioConcurrencyListenerCallback: Instance destroy");
}
