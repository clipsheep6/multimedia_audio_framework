...
void AudioRoutingManagerListenerProxy::OnDistributedRoutingRoleChange(const sptr<AudioDeviceDescriptor> desciptor, const CastType type)
{
    ...
    CHECK_AND_RETURN_LOG(data.WriteInterfaceToken(GetDescriptor()), "OnDistributedRoutingRoleChange: WriteInterfaceToken failed");
