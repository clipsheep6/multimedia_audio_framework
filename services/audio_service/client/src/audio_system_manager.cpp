...
{
    const sptr<IStandardAudioService> gasp = GetAudioSystemManagerProxy();
    CHECK_AND_RETURN_RET_LOG(gasp != nullptr, nullptr, "Audio service unavailable.");
    ...
    std::vector<sptr<AudioDeviceDescriptor>> audioDeviceDescriptors) const
{
    ...
    CHECK_AND_RETURN_RET_LOG(audioRendererFilter->uid >= 0, ERR_INVALID_PARAM, "invalid uid.");
