...
int32_t AudioManagerProxy::SetAsrWhisperMode(AsrWhisperMode asrWhisperMode)
{
    MessageParcel data;
    MessageParcel reply;
    MessageOption option;
    ...
    bool ret = data.WriteInterfaceToken(GetDescriptor());
    CHECK_AND_RETURN_RET_LOG(ret, -1, "WriteInterfaceToken failed");
    data.WriteInt32(static_cast<int32_t>(asrWhisperMode));

    int32_t error = Remote()->SendRequest(static_cast<uint32_t>(AudioServerInterfaceCode::SET_ASR_WHISPER_MODE),
        data, reply, option);
    CHECK_AND_RETURN_RET_LOG(error == ERR_NONE, -1, "failed,error:%d", error);
    int32_t result = reply.ReadInt32();
    return result;
