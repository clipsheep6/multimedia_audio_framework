        ...
        if (processConfig_.audioMode == AUDIO_MODE_PLAYBACK) {
            ...
            if (!needConvert_) {
                ...
                if (bufferHolder == AudioBufferHolder::AUDIO_SERVER_INDEPENDENT) {
                    // Track volume is handled in client loop.
                    size_t dataLength = curWriteBuffer.dataLength;
                    dataLength /= 2; // SAMPLE_S16LE--> 2 byte
                    int16_t *dstPtr = reinterpret_cast<int16_t *>(curWriteBuffer.buffer);
                    for (size_t offset = 0; dataLength > 0; dataLength--) {
                    int32_t sum = 0;
                        int16_t *srcPtr = reinterpret_cast<int16_t *>(bufDesc.buffer) + offset;
...
void AudioProcessInClientInner::ProcessCallbackFucIndependent()
{
    ...
    while (!isCallbackLoopEnd_) {
        ...
        if (streamStatus_->load() == STREAM_RUNNING) {
            ...
            int32_t ret = audioBuffer_->GetWriteBuffer(curWritePos, writeStreamData.bufferDesc);
            if (ret != SUCCESS || writeStreamData.bufferDesc.buffer == nullptr) {
                AUDIO_INFO_LOG("ret is fail or buffer is nullptr");
                return;
            }
            memset_s(writeStreamData.bufferDesc.buffer, writeStreamData.bufferDesc.bufLength, 0, writeStreamData.bufferDesc.bufLength);
        }

        if (!PrepareNextIndependent(curWritePos, wakeUpTime)) {
    ...
    AUDIO_DEBUG_LOG("Client fast Independent stream work loop fuc end, ret %{public}d", ret);
}
