...
int32_t AudioServiceClient::FlushStream()
{
    ...
    while (pa_operation_get_state(operation) == PA_OPERATION_RUNNING) {
        AudioXCollie audioXCollie("AudioServiceClient::PaMainLoopWait", MAINLOOP_WAIT_TIMEOUT_IN_SEC, [this](void *) {
            AUDIO_WARNING_LOG("Call audio server to dump stacktrace");
            static bool triggerDumpStacktrace_ = true;
            if (audioSystemManager_ != nullptr && triggerDumpStacktrace_ == true) {
