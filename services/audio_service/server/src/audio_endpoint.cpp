...
private:
    ...
    static constexpr int64_t READ_FIRST_FRIME_WAIT_NANO = 20000000;
    static constexpr int64_t RECORD_RESYNC_SLEEP_NANO = 2000000;
...
int32_t AudioEndpointInner::PrepareInputDeviceBuffer(const DeviceInfo &deviceInfo)
{
    ...
    int32_t ret = fastSource_->GetMmapBufferInfo(dstBufferFd_, dstTotalSizeInframe_, dstSpanSizeInframe_,
        dstByteSizePerFrame_);
    if (ret != SUCCESS || dstBufferFd_ == -1 || dstTotalSizeInframe_ == 0 || dstSpanSizeInframe_ == 0 ||
        dstByteSizePerFrame_ == 0) {
        AUDIO_ERR_LOG("%{public}s: Get input mmap buffer info fail, ret %{public}d, dstBufferFd %{public}d, \
    ...
    // spanDuration_ may be less than the correct time of dstSpanSizeInframe_.每240点对应的ns时间 = 5ms
    ...
    serverAheadReadTime_ = temp < ONE_MILLISECOND_DURATION ? ONE_MILLISECOND_DURATION : temp; // 服务器提前读取时间 at least 1ms ahead.
...
int32_t AudioEndpointInner::PrepareDeviceBuffer(const DeviceInfo &deviceInfo)
{
    AUDIO_INFO_LOG("%{public}s enter, deviceRole %{public}d.", __func__, deviceInfo.deviceRole);
    if (deviceInfo.deviceRole == INPUT_DEVICE) {
        return PrepareInputDeviceBuffer(deviceInfo);
    }
...
void AudioEndpointInner::RecordReSyncPosition()
{
    ...
    while (true) {
        CHECK_AND_RETURN_LOG(GetDeviceHandleInfo(curHdiWritePos, writeTime),
            "%{public}s get source handle info fail.", __func__);
        if (writeTime > DELTA_TO_REAL_READ_START_TIME)
        {
            break;
        }
    ...
    int64_t correctWriteTime = writeTime - spanDuration_ * followSpanCnt;
    writeTimeModel_.ResetFrameStamp(curHdiWritePos, correctWriteTime);
    uint64_t nextDstReadPos = curHdiWritePos - dstSpanSizeInframe_ * followSpanCnt;
...
int64_t AudioEndpointInner::GetPredictNextWriteTime(uint64_t posInFrame)
{
    ...
    if (handleSpanCout < startPeriodCount || handleSpanCout % oneBigPeriodCount == 0) {
        ...
        if (GetDeviceHandleInfo(writeFrame, writeTime)) {
            uint64_t followSpanCnt = writeFrame % dstSpanSizeInframe_;
            uint64_t correctWriteTime = writeTime - spanDuration_ * followSpanCnt;
            writeTimeModel_.UpdataFrameStamp(writeFrame, correctWriteTime);
        }
    }
...
bool AudioEndpointInner::RecordPrepareNextLoop(uint64_t curReadPos, int64_t &wakeUpTime)
{
    ...
    {
        std::lock_guard<std::mutex> lock(listLock_);
        for (size_t i = 0; i < processBufferList_.size(); i++) {          // processBufferList_是process client的buf
