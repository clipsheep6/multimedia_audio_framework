...
int32_t AudioEndpointInner::GetAdapterBufferInfo(const DeviceInfo &deviceInfo)
{
    ...
    CHECK_AND_RETURN_RET_LOG(ret == SUCCESS && dstBufferFd_ != -1 && dstTotalSizeInframe_ != 0 &&
        dstSpanSizeInframe_ != 0 && dstByteSizePerFrame_ != 0,
        ERR_ILLEGAL_STATE, "get mmap buffer info fail, ret %{public}d, dstBufferFd %{public}d, \
    ...
    AUDIO_DEBUG_LOG("GetAdapterBufferInfo end, fd %{public}d.", dstBufferFd_);
    return SUCCESS;
}

int32_t AudioEndpointInner::PrepareDeviceBuffer(const DeviceInfo &deviceInfo)
{
    AUDIO_INFO_LOG("PrepareDeviceBuffer enter, deviceRole %{public}d.", deviceInfo.deviceRole);
    if (dstAudioBuffer_ != nullptr) {
    ...
    AUDIO_DEBUG_LOG("spanDuration %{public}" PRIu64" ns, serverAheadReadTime %{public}" PRIu64" ns.",
    ...
    CHECK_AND_BREAK_LOG(ret == EOK, "memset buffer fail, ret %{public}d, fd %{public}d.", ret, dstBufferFd_);
    InitAudiobuffer(true);

    AUDIO_DEBUG_LOG("PrepareDeviceBuffer end, fd %{public}d.", dstBufferFd_);
    return SUCCESS;
}
...
bool AudioEndpointInner::StartDevice()
{
    ...
    if (deviceInfo_.deviceRole == INPUT_DEVICE) {
        ...
        CHECK_AND_RETURN_RET_LOG(fastSink_ != nullptr && fastSink_->Start() == SUCCESS,
...
int32_t AudioEndpointInner::ReadFromEndpoint(uint64_t curReadPos)
{
    ...
    CHECK_AND_BREAK_LOG(ret == EOK, "reset buffer fail, ret %{public}d.", ret);
