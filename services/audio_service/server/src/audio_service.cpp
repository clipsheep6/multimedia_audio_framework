...
sptr<AudioProcessInServer> AudioService::GetAudioProcess(const AudioProcessConfig &config)
{
    ...
    std::shared_ptr<OHAudioBuffer> buffer = audioEndpoint->GetEndpointType() == AudioEndpoint::TYPE_INDEPENDENT ? audioEndpoint->GetBuffer() : nullptr;
...
static uint64_t id_ = 1;

std::shared_ptr<AudioEndpoint> AudioService::GetAudioEndpointForDevice(DeviceInfo &deviceInfo, AudioStreamType streamType)
{
    if (deviceInfo.deviceRole == INPUT_DEVICE || deviceInfo.networkId == REMOTE_NETWORK_ID) {
        ...
        if (endpointList_.find(deviceKey) != endpointList_.end()) {
            ...
            std::shared_ptr<AudioEndpoint> endpoint = AudioEndpoint::CreateEndpoint(AudioEndpoint::TYPE_MMAP, 0, streamType, deviceInfo);
        ...
        std::string deviceKey = deviceInfo.networkId + std::to_string(deviceInfo.deviceId) + "_" + std::to_string(id_);
        std::shared_ptr<AudioEndpoint> endpoint = AudioEndpoint::CreateEndpoint(AudioEndpoint::TYPE_INDEPENDENT, id_, streamType, deviceInfo);
