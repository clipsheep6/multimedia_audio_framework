...
int32_t AudioService::OnProcessRelease(IAudioProcessStream *process)
{
    ...
    if (isFind) {
        AUDIO_DEBUG_LOG("find and release process result %{public}d", ret);
    } else {
        AUDIO_DEBUG_LOG("can not find target process, maybe already released.");
    }

    if (needRelease) {
        AUDIO_DEBUG_LOG("find endpoint unlink, call delay release.");
...
std::shared_ptr<AudioEndpoint> AudioService::GetAudioEndpointForDevice(DeviceInfo deviceInfo)
{
    ...
    std::shared_ptr<AudioEndpoint> endpoint = AudioEndpoint::GetInstance(AudioEndpoint::EndpointType::TYPE_MMAP,
        deviceInfo);
    CHECK_AND_RETURN_RET_LOG(endpoint != nullptr, nullptr, "Find no endpoint for the process");
