...
void *AudioServer::paDaemonThread(void *arg)
{
    ...
    AUDIO_DEBUG_LOG("Calling ohos_pa_main\n");
    ohos_pa_main(PA_ARG_COUNT, argv);
    AUDIO_DEBUG_LOG("Exiting ohos_pa_main\n");
    exit(-1);
}
#endif
...
const char *AudioServer::RetrieveCookie(int32_t &size)
{
    ...
    CHECK_AND_RETURN_RET(cookieFile, cookieInfo);
...
int32_t AudioServer::OffloadSetVolume(float volume)
{
    int32_t callingUid = IPCSkeleton::GetCallingUid();
    CHECK_AND_RETURN_RET_LOG(callingUid == audioUid_ || callingUid == ROOT_UID,
        ERR_NOT_SUPPORTED, "OffloadSetVolume refused for %{public}d", callingUid);
    IAudioRendererSink *audioRendererSinkInstance = IAudioRendererSink::GetInstance("offload", "");

    CHECK_AND_RETURN_RET_LOG(audioRendererSinkInstance != nullptr, ERROR,
        "Renderer is null.");
...
sptr<IRemoteObject> AudioServer::CreateAudioProcess(const AudioProcessConfig &config)
{
    ...
    if (callerUid == MEDIA_SERVICE_UID) {
        AUDIO_DEBUG_LOG("Create process for media service.");
        ...
        AUDIO_DEBUG_LOG("Use true client appInfo instead.");
...
int32_t AudioServer::SetParameterCallback(const sptr<IRemoteObject>& object)
{
    ...
    AUDIO_DEBUG_LOG("AudioServer:: SetParameterCallback  done");
