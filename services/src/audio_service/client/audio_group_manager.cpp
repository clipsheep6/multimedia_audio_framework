...
void AudioGroupManager::Init()
{
    ...
    int32_t ret = AudioPolicyManager::GetInstance().GetVolumeGroupInfo(groupId_, volumeGroupInfo);

    if (ret != SUCCESS) {
        AUDIO_ERR_LOG("AudioSystemManager::init GetVolumeGroupInfo failed");
        return;
    }

    netWorkId_ = volumeGroupInfo->networkId_;
}
