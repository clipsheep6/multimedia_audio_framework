...
void AudioPolicyManagerListenerProxy::OnInterrupt(const InterruptAction &interruptAction)
{
    ...
    reply.ReadInt32(); // TODO check if required
}

AudioPolicyManagerListenerCallback::AudioPolicyManagerListenerCallback(const sptr<IStandardAudioPolicyManagerListener> &listener)
    : listener_(listener)
{
}
