...
int32_t AudioStreamCollector::AddRendererStream(AudioStreamChangeInfo &streamChangeInfo)
{
    ...
    rendererStatequeue_[streamChangeInfo.audioRendererChangeInfo.clientUID] =
...
int32_t AudioStreamCollector::RegisterTracker(AudioMode &mode, AudioStreamChangeInfo &streamChangeInfo, const sptr<IRemoteObject> &object)
{
    ...
    CHECK_AND_RETURN_RET_LOG(listener != nullptr, ERR_INVALID_PARAM, "AudioStreamCollector: client tracker obj cast failed");
    std::shared_ptr<AudioClientTracker> callback = std::make_shared<ClientTrackerCallbackListener>(listener);
    CHECK_AND_RETURN_RET_LOG(callback != nullptr, ERR_INVALID_PARAM, "AudioStreamCollector: failed to create tracker cb obj");
    clientTracker_[clientUID] = callback;

    // Kiran:
    AUDIO_INFO_LOG("AudioStreamCollector: Calling OnInit");
    callback->OnInit();
...
int32_t AudioStreamCollector::UpdateRendererStream(AudioStreamChangeInfo &streamChangeInfo)
{
    ...
    if (rendererStatequeue_.find(streamChangeInfo.audioRendererChangeInfo.clientUID) != rendererStatequeue_.end()) {
        if (streamChangeInfo.audioRendererChangeInfo.rendererState ==
            rendererStatequeue_[streamChangeInfo.audioRendererChangeInfo.clientUID]) {
            // Renderer state not changed
            AUDIO_DEBUG_LOG("AudioStreamCollector: UpdateRendererStream playback state not changed clientid %{public}d",
                streamChangeInfo.audioRendererChangeInfo.clientUID);
            return SUCCESS;
        }
    } else {
        AUDIO_INFO_LOG("AudioStreamCollector: UpdateRendererStream client %{public}d not found in rendererStatequeue_",
            streamChangeInfo.audioRendererChangeInfo.clientUID);
    }
            ...
            *it = move(RendererChangeInfo);
            ...
            if (streamChangeInfo.audioRendererChangeInfo.rendererState == RENDERER_RELEASED) {
                ...
                AUDIO_DEBUG_LOG("AudioStreamCollector: Session removed for client %{public}d session %{public}d",
                    streamChangeInfo.audioRendererChangeInfo.clientUID,
                    streamChangeInfo.audioRendererChangeInfo.sessionId);
                rendererStatequeue_.erase(streamChangeInfo.audioRendererChangeInfo.clientUID);
            }
            return SUCCESS;
        }
    }
    AUDIO_INFO_LOG("AudioStreamCollector: UpdateRendererStream: clientUI not in audioRendererChangeInfos_:%{public}d",
        streamChangeInfo.audioRendererChangeInfo.clientUID);
    return SUCCESS;
}
...
int32_t AudioStreamCollector::UpdateCapturerStream(AudioStreamChangeInfo &streamChangeInfo)
{
    ...
    if (capturerStatequeue_.find(streamChangeInfo.audioCapturerChangeInfo.clientUID) != capturerStatequeue_.end()) {
        if (streamChangeInfo.audioCapturerChangeInfo.capturerState ==
            capturerStatequeue_[streamChangeInfo.audioCapturerChangeInfo.clientUID]) {
            // Capturer state not changed
            AUDIO_DEBUG_LOG("AudioStreamCollector: UpdateCapturerStream playback state not changed clientid %{public}d",
            ...
            *it = move(CapturerChangeInfo);
            ...
            if (streamChangeInfo.audioCapturerChangeInfo.capturerState ==  CAPTURER_RELEASED) {
                ...
                AUDIO_DEBUG_LOG("UpdateCapturerStream::Session is removed for client %{public}d session %{public}d",
                    streamChangeInfo.audioCapturerChangeInfo.clientUID,
                    streamChangeInfo.audioCapturerChangeInfo.sessionId);
                capturerStatequeue_.erase(streamChangeInfo.audioCapturerChangeInfo.clientUID);
            }
        return SUCCESS;
        }
    }
...
void AudioStreamCollector::DisplayInternalStreamInfo()
{
    ...
    for (auto const &pair: rendererStatequeue_) {
        AUDIO_DEBUG_LOG("clientUID: %{public}d  RendererState : %{public}d", pair.first, pair.second);
    }
...
int32_t AudioStreamCollector::GetCurrentRendererChangeInfos(vector<unique_ptr<AudioRendererChangeInfo>> &rendererChangeInfos)
{
    DisplayInternalStreamInfo();
...
int32_t AudioStreamCollector::GetCurrentCapturerChangeInfos(vector<unique_ptr<AudioCapturerChangeInfo>> &capturerChangeInfos)
{
