...
int32_t AudioPolicyServer::RegisterTracker(AudioMode &mode, AudioStreamChangeInfo &streamChangeInfo, const sptr<IRemoteObject> &object)
{
    ...
    RegisterClientDeathRecipient(object);
...
int32_t AudioPolicyServer::GetCurrentRendererChangeInfos(vector<unique_ptr<AudioRendererChangeInfo>> &audioRendererChangeInfos)
{
    return mStreamCollector.GetCurrentRendererChangeInfos(audioRendererChangeInfos);
}

int32_t AudioPolicyServer::GetCurrentCapturerChangeInfos(vector<unique_ptr<AudioCapturerChangeInfo>> &audioCapturerChangeInfos)
{
    return mStreamCollector.GetCurrentCapturerChangeInfos(audioCapturerChangeInfos);
}

void AudioPolicyServer::RegisterClientDeathRecipient(const sptr<IRemoteObject> &object)
{
    ...
    if (deathRecipient_ != nullptr) {
        deathRecipient_->SetNotifyCb(std::bind(&AudioPolicyServer::RegisteredClientDied, this, std::placeholders::_1));
        bool result = object->AddDeathRecipient(deathRecipient_);
        if (!result) {
            AUDIO_ERR_LOG("failed to add deathRecipient");
        }
...
void AudioPolicyServer::RegisteredClientDied(pid_t pid)
{
    AUDIO_INFO_LOG("RegisteredClient died: remove entry, pid %{public}d", pid);
    mStreamCollector.RegisteredClientDied(static_cast<int32_t>(pid));
}
} // namespace AudioStandard
} // namespace OHOS
