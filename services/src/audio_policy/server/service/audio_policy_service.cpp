    ...
    DeviceType deviceType)
{
    ...
    audioModuleInfo.fileName = "remote_" + networkId;
    ...
    ConnectType connectType, bool connected)
{
    ...
    if (type == GroupType::VOLUME_TYPE) {
        ...
        for (iter = mVolumeGroups.begin(); iter != mVolumeGroups.end();) {
            if ((*iter)->groupName_ == groupName && (*iter)->networkId_ == networkId) {
                volumeGroupInfo = *iter;
                groupId = volumeGroupInfo->volumeGroupId_;
                // if status is disconnected, remove the group that has none audio device
                if (!connected && GetDevicesForGroup(type, groupId).size() == 0) {
        ...
        if (volumeGroupInfo == nullptr && groupName != GROUP_NAME_NONE) {
            ...
            volumeGroupInfo = new(std::nothrow) VolumeGroupInfo(groupId, NO_REMOTE_ID, groupName, networkId,
                connectType);
            mVolumeGroups.push_back(volumeGroupInfo);
        }
    } else {
        sptr<InterruptGroupInfo> interruptGroupInfo;
        std::vector<sptr<InterruptGroupInfo>>::iterator iter;
        for (iter = mInterruptGroups.begin(); iter != mInterruptGroups.end();) {
            if ((*iter)->groupName_ == groupName && (*iter)->networkId_ == networkId) {
                interruptGroupInfo = *iter;
                groupId = interruptGroupInfo->interruptGroupId_;
                // if status is disconnected, remove the group that has none audio device
                if (!connected && GetDevicesForGroup(type, groupId).size() == 0) {
        ...
        if (interruptGroupInfo == nullptr && groupName != GROUP_NAME_NONE) {
            groupId = AudioGroupHandle::GetInstance().GetNextId(type);
            interruptGroupInfo = new(std::nothrow) InterruptGroupInfo(groupId, NO_REMOTE_ID, groupName, networkId,
                connectType);
...
std::vector<sptr<OHOS::AudioStandard::AudioDeviceDescriptor>> AudioPolicyService::GetDevicesForGroup(GroupType type, int32_t groupId)
{
...
void AudioPolicyService::UpdateInterruptGroupInfo(sptr<InterruptGroupInfo>& info, DStatusInfo& statusInfo)
{
    ...
    mInterruptGroups.push_back(info);
}
