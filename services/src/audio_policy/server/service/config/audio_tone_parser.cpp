...
int32_t AudioToneParser::LoadConfig(std::unordered_map<int32_t, std::shared_ptr<ToneInfo>> &toneDescriptorMap)
{
    ...
    while (currNode != nullptr) {
        if ((currNode->type == XML_ELEMENT_NODE) && (!xmlStrcmp(currNode->name, reinterpret_cast<const xmlChar*>("Tones")))) {
            currNode = currNode->xmlChildrenNode;
        } else if ((currNode->type == XML_ELEMENT_NODE) && (!xmlStrcmp(currNode->name, reinterpret_cast<const xmlChar*>("ToneInfo")))) {
...
void AudioToneParser::ParseToneInfoAttribute(xmlNode *sNode, std::shared_ptr<ToneInfo> ltoneDesc)
{
    ...
    while (sNode != nullptr) {
        ...
        if (!xmlStrcmp(sNode->name, reinterpret_cast<const xmlChar*>("RepeatCount"))) {
            ...
            lpValue = reinterpret_cast<char*>(xmlGetProp(sNode,
                reinterpret_cast<xmlChar*>(const_cast<char*>("value"))));
            if (!xmlStrcmp(reinterpret_cast<const xmlChar*>(lpValue), reinterpret_cast<const xmlChar*>("INF"))) {
                ltoneDesc->repeatCnt = TONEINFO_INF;
            } else {
                ltoneDesc->repeatCnt = atoi(lpValue);
            }
            ...
            lpValue = reinterpret_cast<char*>(xmlGetProp(sNode,
                reinterpret_cast<xmlChar*>(const_cast<char*>("value"))));
            ltoneDesc->repeatSegment = atoi(lpValue);
    ...
    std::shared_ptr<ToneInfo>> &toneDescriptorMap)
{
    ...
    while (currNode != nullptr) {
        ...
        if (!xmlStrcmp(currNode->name, reinterpret_cast<const xmlChar*>("ToneInfo"))) {
            ...
            char *lpToneType = reinterpret_cast<char*>(xmlGetProp(currNode,
                reinterpret_cast<xmlChar*>(const_cast<char*>("toneType"))));
            int toneType = atoi(lpToneType);
