...
int32_t AudioPolicyProxy::UnSetAudioManagerCallback(const AudioStreamType streamType)
{
    ...
    if (error != ERR_NONE) {
        MEDIA_ERR_LOG("unset callback failed, error: %{public}d", error);
        return error;
    }
...
int32_t AudioPolicyProxy::ActivateAudioInterrupt(const AudioInterrupt &audioInterrupt)
{
    ...
    if (error != ERR_NONE) {
        MEDIA_ERR_LOG("activate interrupt failed, error: %{public}d", error);
        return error;
    }
...
int32_t AudioPolicyProxy::DeactivateAudioInterrupt(const AudioInterrupt &audioInterrupt)
{
    ...
    if (error != ERR_NONE) {
        MEDIA_ERR_LOG("deactivate interrupt failed, error: %{public}d", error);
        return error;
    }
