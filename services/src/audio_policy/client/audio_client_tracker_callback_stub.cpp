    ...
    uint32_t code, MessageParcel &data, MessageParcel &reply, MessageOption &option)
{
    ...
    switch (code) {
        ...
        default: {
            AUDIO_ERR_LOG("AudioClientTrackerCallbackStub: Default Hit!!");
            reply.WriteInt32(-1);
            break;
        }
    }
...
void AudioClientTrackerCallbackStub::OnInit()
{
    AUDIO_DEBUG_LOG("AudioClientTrackerCallbackStub::OnInit");
    std::shared_ptr<AudioClientTracker> cb = callback_.lock();
    if (cb != nullptr) {
        AUDIO_DEBUG_LOG("AudioClientTrackerCallbackStub::OnInit CALLBACK NOT NULL");
        cb->OnInit();
    } else {
        AUDIO_DEBUG_LOG("AudioClientTrackerCallbackStub::OnInit CALLBACK NULL");
    }
}
