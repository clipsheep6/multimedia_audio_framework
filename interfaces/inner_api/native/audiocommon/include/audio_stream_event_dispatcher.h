...
//public:
    ...
    //AudioStreamChangeInfo audioStreamChangeInfo;
//private:
    //int32_t clientUID;
};
...
public:
    ...
    void SendRendererInfoEventToDispatcher(AudioMode &mode, std::vector<std::unique_ptr<AudioRendererChangeInfo>> &audioRendererChangeInfos);
    void SendCapturerInfoEventToDispatcher(AudioMode &mode, std::vector<std::unique_ptr<AudioCapturerChangeInfo>> &audioCapturerChangeInfos);
...
private:
    ...
    //std::queue<StreamStateChangeRequest> streamStateChangeQueue_;
...
static constexpr uint32_t MAX_THREAD = 1;  // to be updated
