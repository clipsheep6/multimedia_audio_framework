...
{
    if (rawData == nullptr || size < LIMITSIZE ||
        size < (sizeof(int32_t)+sizeof(SourceType)))
    {
        return;
    }
...
{
    if (rawData == nullptr || size < LIMITSIZE ||
        size < (sizeof(uint32_t)+sizeof(AudioPermissionState))) {
        return;
    }
...
void AdjustStereoToMonoForPCM8BitFuzzTest(const uint8_t *rawData, size_t size)
{
    ...
    int8_t value = *const_cast<int8_t*>(reinterpret_cast<const int8_t*>(rawData));
    int8_t arr[] = {value, value, value};
    int8_t *data = arr;
...
{
    if (rawData == nullptr || size < LIMITSIZE ||
        size < (sizeof(uint64_t)+sizeof(int8_t*)+sizeof(float)+
        sizeof(float))) {
        return;
    }
    int8_t *data = const_cast<int8_t*>(reinterpret_cast<const int8_t*>(rawData));
...
{
    if (rawData == nullptr || size < LIMITSIZE ||
        size < (sizeof(uint64_t)+sizeof(int16_t*)+sizeof(float)+
        sizeof(float))) {
        return;
    }
    int16_t *data = const_cast<int16_t*>(reinterpret_cast<const int16_t*>(rawData));
...
{
    if (rawData == nullptr || size < LIMITSIZE ||
        size < (sizeof(uint64_t)+sizeof(int32_t*)+sizeof(float)+
        sizeof(float))) {
        return;
    }
    int32_t *data = const_cast<int32_t*>(reinterpret_cast<const int32_t*>(rawData));
...
{
    if (rawData == nullptr || size < LIMITSIZE ||
        size < (sizeof(unsigned)+sizeof(uint8_t*)+sizeof(float))) {
        return;
    }
    unsigned n = *(reinterpret_cast<const unsigned*>(rawData));
...
{
    if (rawData == nullptr || size < LIMITSIZE ||
        size < (sizeof(unsigned)+sizeof(int32_t*)+sizeof(float))) {
        return;
    }
    unsigned n = *(reinterpret_cast<const unsigned*>(rawData));
    rawData += sizeof(unsigned);
...
void UpdateMaxAmplitudeFuzzTest(const uint8_t * rawData, size_t size)
{
    if (rawData == nullptr || size == 0 ||
        size < (sizeof(ConvertHdiFormat)+sizeof(char*)+sizeof(uint64_t))) {
        return;
    }
    ConvertHdiFormat format = *(reinterpret_cast<const ConvertHdiFormat*>(rawData));
    rawData += sizeof(ConvertHdiFormat);
...
{
    if (rawData == nullptr || size == 0 ||
        size < (sizeof(int8_t *)+sizeof(uint64_t))) {
        return;
    }
...
{
    if (rawData == nullptr || size == 0 ||
        size < (sizeof(int16_t *)+sizeof(uint64_t))) {
        return;
    }
...
{
    if (rawData == nullptr || size == 0 ||
        size < (sizeof(char *)+sizeof(uint64_t))) {
        return;
    }
    char *frame = const_cast<char*>(reinterpret_cast<const char*>(rawData));
    rawData += sizeof(char*);
...
{
    if (rawData == nullptr || size == 0 ||
        size < (sizeof(int32_t *)+sizeof(uint64_t))) {
        return;
    }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::AudioStandard::AdjustAudioBalanceForPCM24BitFuzzTest(data, size);
    OHOS::AudioStandard::AdjustAudioBalanceForPCM32BitFuzzTest(data, size);
    OHOS::AudioStandard::ConvertFrom24BitToFloatFuzzTest(data, size);
    OHOS::AudioStandard::ConvertFrom32BitToFloatFuzzTest(data, size);
    OHOS::AudioStandard::ConvertFromFloatTo24BitFuzzTest(data, size);
    OHOS::AudioStandard::ConvertFromFloatTo32BitFuzzTest(data, size);
    OHOS::AudioStandard::UpdateMaxAmplitudeFuzzTest(data, size);
    OHOS::AudioStandard::CalculateMaxAmplitudeForPCM8BitFuzzTest(data, size);
    OHOS::AudioStandard::CalculateMaxAmplitudeForPCM16BitFuzzTest(data, size);
    OHOS::AudioStandard::CalculateMaxAmplitudeForPCM24BitFuzzTest(data, size);
    OHOS::AudioStandard::CalculateMaxAmplitudeForPCM32BitFuzzTest(data, size);
