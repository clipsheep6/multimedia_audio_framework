...
{
    if (rawData == nullptr || size < LIMITSIZE ||
        size < (sizeof(DeviceType)+sizeof(DeviceFlag)+sizeof(BluetoothOffloadState))) {
        return;
    }
...
{
    if (rawData == nullptr || size < LIMITSIZE ||
        size < 2*(sizeof(DeviceType)+sizeof(DeviceFlag))+sizeof(BluetoothOffloadState)) {
        return;
    }
...
{
    if (rawData == nullptr || size < LIMITSIZE ||
        size < (sizeof(bool)+sizeof(int32_t))) {
        return;
    }
...
void AudioCreateAudioProcessTest(const uint8_t* rawData, size_t size)
{
    ...
    MessageParcel data;
    data.WriteInterfaceToken(FORMMGR_INTERFACE_TOKEN);
    cout<<"START AUDIOCREATEAUDIOPROCESS TEST"<<endl;
    ...
    audioServer->OnRemoteRequest(static_cast<uint32_t>(AudioServerInterfaceCode::CREATE_AUDIOPROCESS),
        data, reply, option);
    cout<<"END AUDIOCREATEAUDIOPROCESS TEST"<<endl;
}
...
void AudioRendererInServerTestFirst(const uint8_t* rawData, size_t size, std::shared_ptr<RendererInServer> renderer)
{
    ...
    operation_int = (operation_int%13) - 1;
