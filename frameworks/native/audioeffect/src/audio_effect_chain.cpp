...
#ifdef SENSOR_ENABLE
AudioEffectChain::AudioEffectChain(std::string scene, AudioSpatialDeviceType spatialDeviceType,
...
int32_t AudioEffectChain::SetEffectParamToHandle(AudioEffectHandle handle, int32_t &replyData)
{
    ...
    // Set param
    AudioEffectParam *effectParam{ nullptr };
    effectParam = new (std::nothrow) AudioEffectParam;
    if (effectParam == nullptr) {
        AUDIO_ERR_LOG("Failed to allocate memory for effectParam");
        return ERROR;
    }
    effectParam->data = new (std::nothrow) int32_t[NUM_SET_EFFECT_PARAM];
    if (effectParam->data == nullptr) {
        AUDIO_ERR_LOG("Failed to allocate memory for effectParam->data");
        return ERROR;
    }
...
int32_t AudioEffectChain::UpdateSpatialDeviceType(AudioSpatialDeviceType spatialDeviceType)
{
    spatialDeviceType_ = spatialDeviceType;

    int32_t ret = UpdateEffectParam();
    CHECK_AND_RETURN_RET_LOG(ret == 0, ERROR, "UpdateEffectParam failed.");
