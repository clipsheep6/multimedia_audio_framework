...
static void MemcpyToI32FromI24(uint8_t *src, int32_t *dst, size_t count)
{
    for (size_t i = 0; i < count; i++) {
        ...
        *(dst + i) = static_cast<int32_t>((static_cast<uint32_t>(tmp[2]) << (2 * sizeof(uint8_t))) |
            (static_cast<uint32_t>(tmp[1]) << sizeof(uint8_t)) |
            static_cast<uint32_t>(tmp[0])); // 2 is the offset to the highest byte in a 24bit integer;
    }
}
