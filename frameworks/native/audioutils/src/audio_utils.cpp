...
Trace::Trace(const std::string &value, bool isShowLog, bool isEnable)
{
    ...
    if (isShowLog) {
        isShowLog_ = true;
        AUDIO_DEBUG_LOG("%{public}s start.", value_.c_str());
    }
...
void Trace::End()
{
    ...
    if (!isFinished_) {
        ...
        isFinished_ = true;
        if (isShowLog_) {
            AUDIO_DEBUG_LOG("%{public}s end.", value_.c_str());
        }
    }
#endif
}
...
{
    AUDIO_INFO_LOG("Start AudioXCollie, tag: %{public}s, timeoutSeconds: %{public}u",
...
{
    CHECK_AND_RETURN_RET_LOG(key != nullptr, false, "GetSysPara: key is nullptr");
    ...
    AUDIO_DEBUG_LOG("GetSysPara: key:%{public}s value:%{public}s", key, paraValue);
