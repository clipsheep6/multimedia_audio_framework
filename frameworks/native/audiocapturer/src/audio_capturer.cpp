...
int32_t AudioCapturerPrivate::UnregisterAudioCapturerEventListener()
{
    if (audioStateChangeCallback_->DeviceChangeCallbackArraySize() == 0 &&
        audioStateChangeCallback_->CapturerInfoChangeCallbackArraySize() == 0) {
        if (audioStateChangeCallback_ != nullptr) {
            int32_t ret =
                AudioPolicyManager::GetInstance().UnregisterAudioCapturerEventListener(getpid());
            if (ret != 0) {
                AUDIO_ERR_LOG("AudioCapturerPrivate::UnregisterAudioCapturerEventListener failed");
                return ERROR;
            }
...
int32_t AudioCapturerStateChangeCallbackImpl::CapturerInfoChangeCallbackArraySize()
{
    ...
    const std::vector<std::unique_ptr<AudioCapturerChangeInfo>> &audioCapturerChangeInfos)
{
    ...
    if (ret) {
        AUDIO_ERR_LOG("OnCapturerStateChange: Get sessionId failed");
        return;
    }
