...
{
    AUDIO_INFO_LOG("SetSupportStreamUsage");
    ...
    for (size_t i = 0; i < usage.size(); i++) {
        ...
        AUDIO_DEBUG_LOG("SetSupportStreamUsage:%{public}d", usage[i]);
    }
}

bool PlaybackCapturerManager::IsStreamSupportInnerCapturer(int32_t streamUsage)
{
    AUDIO_DEBUG_LOG("IsStreamSupportInnerCapturer, streamUsage:%{public}d", streamUsage);
    if (supportStreamUsageSet_.empty()) {
        return streamUsage == STREAM_USAGE_MEDIA;
    }
    if (supportStreamUsageSet_.find(streamUsage) != supportStreamUsageSet_.end()) {
        return true;
    }
    return false;
}
...
{
    AUDIO_INFO_LOG("IsPrivacySupportInnerCapturer, privacyType:%{public}d", privacyType);
    if (privacyType == PRIVACY_TYPE_PUBLIC) {
        return true;
    }
    return false;
}
