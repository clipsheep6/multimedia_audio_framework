...
PA_MODULE_DESCRIPTION("Virtual channel effecting sink");
...
//     pa_memblockq_peek_fixed_size(u->bufInQ, u->frameLen * 2 * 2, &tchunk);
//     pa_memblockq_drop(u->bufInQ, tchunk.length);

//     chunk->index = 0;
//     chunk->length = u->frameLen * 2 * 2;
...
int pa__init(pa_module *m) {
    ...
    if (!(sink_data.name = pa_xstrdup(pa_modargs_get_value(ma, "sink_name", NULL))))
        sink_data.name = pa_sprintf_malloc("%s.effected", master->name);
