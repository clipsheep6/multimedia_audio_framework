...
static void update_minimum_latency(struct userdata *u, pa_sink *sink, bool print_msg) {
    ...
    if (print_msg) {
        ...
        if (u->latency < u->minimum_latency)
            AUDIO_WARNING_LOG("Configured latency of %0.2f ms is smaller than minimum latency, using minimum instead", (double)u->latency / PA_USEC_PER_MSEC);
    }
}

/* Called from main context */
static void adjust_rates(struct userdata *u) {
    AUDIO_INFO_LOG("adjust_rates");
    size_t buffer;
    ...
    if (u->underrun_counter > 2) {
        ...
        if (u->max_latency == 0 || target_latency < u->max_latency) {
            ...
            AUDIO_WARNING_LOG("Too many underruns, increasing latency to %0.2f ms", (double)target_latency / PA_USEC_PER_MSEC);
        } else {
            u->underrun_latency_limit = PA_CLIP_SUB((int64_t)u->max_latency, u->sink_latency_offset + u->source_latency_offset);
            AUDIO_WARNING_LOG("Too many underruns, configured maximum latency of %0.2f ms is reached", (double)u->max_latency / PA_USEC_PER_MSEC);
    ...
    if (u->fast_adjust_threshold > 0 && abs(latency_difference) > u->fast_adjust_threshold) {
        AUDIO_DEBUG_LOG ("Latency difference larger than %" PRIu64 " msec, skipping or inserting samples.", u->fast_adjust_threshold / PA_USEC_PER_MSEC);
...
static void memblockq_adjust(struct userdata *u, int64_t latency_offset_usec, bool allow_push) {
    // AUDIO_INFO_LOG("memblockq_adjust");
    ...
    if (current_memblockq_length > requested_memblockq_length) {
        ...
        AUDIO_INFO_LOG("Dropping %" PRIu64 " usec of audio from queue", pa_bytes_to_usec(buffer_correction, &u->sink_input->sample_spec));
        ...
        AUDIO_INFO_LOG("Adding %" PRIu64 " usec of silence to queue", pa_bytes_to_usec(buffer_correction, &u->sink_input->sample_spec));
...
static void source_output_push_cb(pa_source_output *o, const pa_memchunk *chunk) {
    // AUDIO_INFO_LOG("source_output_push_cb, source output of source:%{public}s", o->source->name);
    struct userdata *u;
    pa_usec_t push_time;
...
static int source_output_process_msg_cb(pa_msgobject *obj, int code, void *data, int64_t offset, pa_memchunk *chunk) {
    ...
    // AUDIO_INFO_LOG("source_output_process_msg_cb, code:%{public}d", code);
...
static void set_source_output_latency(struct userdata *u, pa_source *source) {
    ...
    if (u->configured_source_latency != requested_latency)
        AUDIO_WARNING_LOG("Cannot set requested source latency of %0.2f ms, adjusting to %0.2f ms", (double)requested_latency / PA_USEC_PER_MSEC, (double)u->configured_source_latency / PA_USEC_PER_MSEC);
}

/* Called from input thread context */
static void source_output_attach_cb(pa_source_output *o) {
    // AUDIO_INFO_LOG("source_output_attach_cb, source:%{public}s", o->source->name);
    struct userdata *u;
...
static void source_output_detach_cb(pa_source_output *o) {
    // AUDIO_INFO_LOG("source_output_detach_cb, source:%{public}s", o->source->name);
    struct userdata *u;
...
static void source_output_kill_cb(pa_source_output *o) {
    // AUDIO_INFO_LOG("source_output_kill_cb, source:%{public}s", o->source->name);
    struct userdata *u;
...
static int sink_input_pop_cb(pa_sink_input *i, size_t nbytes, pa_memchunk *chunk) {
    // AUDIO_INFO_LOG("sink_input_pop_cb, idx:%{public}u, sink:%{public}s, bytes:%{public}zu", i->index, i->sink->name, nbytes);
    struct userdata *u;
    ...
    if (pa_memblockq_peek(u->memblockq, chunk) < 0) {
        // AUDIO_INFO_LOG("Could not peek into queue");
        return -1;
    }
...
static int sink_input_process_msg_cb(pa_msgobject *obj, int code, void *data, int64_t offset, pa_memchunk *chunk) {
    ...
    // AUDIO_INFO_LOG("sink_input_process_msg_cb, code:%{public}d", code);
...
static void set_sink_input_latency(struct userdata *u, pa_sink *sink) {
    ...
    if (u->configured_sink_latency != requested_latency)
        AUDIO_WARNING_LOG("Cannot set requested sink latency of %0.2f ms, adjusting to %0.2f ms", (double)requested_latency / PA_USEC_PER_MSEC, (double)u->configured_sink_latency / PA_USEC_PER_MSEC);
}

/* Called from output thread context */
static void sink_input_attach_cb(pa_sink_input *i) {
    // AUDIO_INFO_LOG("sink_input_attach_cb, sink:%{public}s", i->sink->name);
    struct userdata *u;
...
static void sink_input_detach_cb(pa_sink_input *i) {
    // AUDIO_INFO_LOG("sink_input_detach_cb, sink:%{public}s", i->sink->name);
    struct userdata *u;
...
static void sink_input_kill_cb(pa_sink_input *i) {
    // AUDIO_INFO_LOG("sink_input_kill_cb, sink:%{public}s", i->sink->name);
    struct userdata *u;
...
static void sink_input_state_change_cb(pa_sink_input *i, pa_sink_input_state_t state) {
    // AUDIO_INFO_LOG("sink_input_state_change_cb, sink:%{public}s, state:%{public}d", i->sink->name, state);
    struct userdata *u;
...
static int loopback_process_msg_cb(pa_msgobject *o, int code, void *userdata, int64_t offset, pa_memchunk *chunk) {
    ...
    // AUDIO_INFO_LOG("loopback_process_msg_cb, code:%{public}d", code);
    ...
    switch (code) {
        ...
        case LOOPBACK_MESSAGE_SOURCE_LATENCY_RANGE_CHANGED:
            ...
            if (current_latency > u->configured_source_latency) {
                ...
                AUDIO_WARNING_LOG("Source minimum latency increased to %0.2f ms", (double)current_latency / PA_USEC_PER_MSEC);
        ...
        case LOOPBACK_MESSAGE_SINK_LATENCY_RANGE_CHANGED:
            ...
            if (current_latency > u->configured_sink_latency) {
                 ...
                 * implementations. */
                AUDIO_WARNING_LOG("Sink minimum latency increased to %0.2f ms", (double)current_latency / PA_USEC_PER_MSEC);
...
{
    // AUDIO_INFO_LOG("sink_port_latency_offset_changed_cb");
...
{
    // AUDIO_INFO_LOG("source_port_latency_offset_changed_cb");
