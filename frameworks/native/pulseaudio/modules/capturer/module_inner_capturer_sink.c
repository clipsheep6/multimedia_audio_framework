...
static void ProcessRewind(struct userdata *u, pa_usec_t now) {
    ...
    AUDIO_DEBUG_LOG("Requested to rewind %lu bytes.", (unsigned long) rewind_nbytes);
    ...
    AUDIO_DEBUG_LOG("Rewound %lu bytes.", (unsigned long) rewind_nbytes);
    return;
...
int pa__init(pa_module *m) {
    ...
    if (!(u->thread = pa_thread_new("inner-capturer-sink", thread_func, u))) {
        AUDIO_ERR_LOG("Failed to create thread.");
        return InitFailed(m, ma);
    }
    pa_sink_set_latency_range(u->sink, 0, u->buffer_size);
