...
RemoteFastAudioRendererSink *RemoteFastAudioRendererSink::GetInstance(const char *deviceNetworkId)
{
    AUDIO_INFO_LOG("GetInstance.");
    RemoteFastAudioRendererSinkInner *audioRenderer_ = nullptr;
    if (deviceNetworkId == nullptr) {
        return audioRenderer_;
    }
    // check if it is in our map
    std::string deviceName = deviceNetworkId;
    if (allRFSinks.count(deviceName)) {
        return allRFSinks[deviceName];
    } else {
        audioRenderer_ = new(std::nothrow) RemoteFastAudioRendererSinkInner(deviceName);
        AUDIO_DEBUG_LOG("new Daudio device sink:[%{public}s]", deviceNetworkId);
        allRFSinks[deviceName] = audioRenderer_;
    }
...
{
#ifdef PRODUCT_M40
    AUDIO_INFO_LOG("RemoteFastAudioRendererSink::SetParameter: key %{public}d, condition: %{public}s, value: %{public}s",
        key, condition.c_str(), value.c_str());
    enum AudioExtParamKey hdiKey = AudioExtParamKey(key);
    if (audioAdapter_ == nullptr) {
        AUDIO_ERR_LOG("RemoteFastAudioRendererSink::SetAudioParameter audioAdapter_ is null");
        return;
    }
    int32_t ret = audioAdapter_->SetExtraParams(audioAdapter_, hdiKey, condition.c_str(), value.c_str());
    if (ret != SUCCESS) {
        AUDIO_ERR_LOG("RemoteFastAudioRendererSink::SetAudioParameter failed, error code: %d", ret);
    }
#endif
}
...
{
#ifdef PRODUCT_M40
    AUDIO_INFO_LOG("RemoteFastAudioRendererSink::GetParameter: key %{public}d, condition: %{public}s", key,
        condition.c_str());
    enum AudioExtParamKey hdiKey = AudioExtParamKey(key);
    char value[PARAM_VALUE_LENTH];
    if (audioAdapter_ == nullptr) {
        AUDIO_ERR_LOG("RemoteFastAudioRendererSink::GetAudioParameter audioAdapter_ is null");
        return "";
    }
    int32_t ret = audioAdapter_->GetExtraParams(audioAdapter_, hdiKey, condition.c_str(), value, PARAM_VALUE_LENTH);
    if (ret !=SUCCESS) {
        AUDIO_ERR_LOG("RemoteFastAudioRendererSink::GetAudioParameter failed, error code: %d", ret);
        return "";
    }
    return value;
#else
...
int32_t RemoteFastAudioRendererSinkInner::PrepareMmapBuffer()
{
    ...
    CHECK_AND_RETURN_RET_LOG((ret == SUCCESS), ERR_OPERATION_FAILED,
        "%{public}s require mmap buffer failed, ret:%{public}d.", ret);
...
int32_t RemoteFastAudioRendererSinkInner::GetMmapBufferInfo(int &fd, uint32_t &totalSizeInframe, uint32_t &spanSizeInframe,
...
int32_t RemoteFastAudioRendererSinkInner::GetMmapHandlePosition(uint64_t &frames, int64_t &timeSec, int64_t &timeNanoSec)
{
