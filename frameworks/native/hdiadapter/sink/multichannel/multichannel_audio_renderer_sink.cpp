...
const uint32_t MULTICHANNEL_OUTPUT_STREAM_ID = 13; // 13 + 0 * 8
...
public:
    ...
    explicit MultiChannelRendererSinkInner(const std::string &halName = "primary");
...
{
    if (halName == "usb") {
        static MultiChannelRendererSinkInner audioRendererUsb(halName);
...
int32_t MultiChannelRendererSinkInner::Start(void)
{
    ...
    if (!started_) {
        ...
        ret = audioRender->GetFrameSize(audioRender_, &frameSize);
        if (ret) {
            return ERR_NOT_STARTED;
        }
        ret = audioRender->GetFrameCount(audioRender_, &frameCount);
        if (ret) {
            return ERR_NOT_STARTED;
        }
