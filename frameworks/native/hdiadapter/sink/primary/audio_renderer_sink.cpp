...
{
    CHECK_AND_RETURN_RET_LOG(!outputDevices.empty() && outputDevices.size() <= AUDIO_ACTIVE_DEVICES_LIMIT,
...
{
    CHECK_AND_RETURN_RET_LOG(!activeDevices.empty() && activeDevices.size() <= AUDIO_ACTIVE_DEVICES_LIMIT,
        ERR_INVALID_PARAM, "Invalid audio devices.");
    ...
    if (openSpeaker_) {
        ...
        if (activeDevices.size() != currentDevicesSize_ ||
            activeDevice != currentActiveDevice_ ||
            (isAudioSceneUpdate &&
