...
int32_t FastAudioRendererSinkInner::PrepareMmapBuffer()
{
    ...
    AUDIO_DEBUG_LOG("AudioMmapBufferDescriptor memoryAddress[%{private}p] memoryFd[%{public}d] totalBufferFrames"
...
void FastAudioRendererSinkInner::PreparePosition()
{
    ...
    AUDIO_DEBUG_LOG("First render frame start with curReadPos_[%{public}d] curWritePos_[%{public}d]", curReadPos_,
        curWritePos_);
#endif
}
...
int32_t FastAudioRendererSinkInner::RenderFrame(char &data, uint64_t len, uint64_t &writeLen)
{
    ...
    CHECK_AND_RETURN_LOG(audioRender_ != nullptr, ERR_INVALID_HANDLE, "Audio Render Handle is nullptr!");
...
void FastAudioRendererSinkInner::KeepRunningLock()
{
    ...
    if (keepRunningLock_ != nullptr) {
        AUDIO_DEBUG_LOG("FastAudioRendererSink call KeepRunningLock lock");
...
{
    if (keepRunningLock_ != nullptr) {
        AUDIO_DEBUG_LOG("FastAudioRendererSink call KeepRunningLock UnLock");
...
{
    CHECK_AND_RETURN_RET_LOG(audioRender_ != nullptr, ERR_INVALID_HANDLE,
        "FastAudioRendererSink: GetLatency failed audio render null");

    CHECK_AND_RETURN_RET_LOG(latency, ERR_INVALID_PARAM,
        "FastAudioRendererSink: GetLatency failed latency null");
...
int32_t FastAudioRendererSinkInner::Stop(void)
{
    ...
    CHECK_AND_RETURN_RET_LOG(audioRender_ != nullptr, ERR_INVALID_HANDLE,
        "FastAudioRendererSink::Stop failed audioRender_ null");
#ifdef FEATURE_POWER_MANAGER
    KeepRunningUnlock();
#endif
    ...
    if (started_) {
        ...
        CHECK_AND_RETURN_RET_LOG(ret == 0, ERR_OPERATION_FAILED,
            "FastAudioRendererSink::Stop failed! ret: %{public}d.", ret);
    }
    started_ = false;
...
int32_t FastAudioRendererSinkInner::Pause(void)
{
    ...
    CHECK_AND_RETURN_RET_LOG(audioRender_ != nullptr, ERR_INVALID_HANDLE,
        "FastAudioRendererSink::Pause failed audioRender_ null");

    CHECK_AND_RETURN_RET_LOG(started_, ERR_OPERATION_FAILED,
        "FastAudioRendererSink::Pause invalid state!");

    if (!paused_) {
        ret = audioRender_->Pause(audioRender_);
        CHECK_AND_RETURN_RET_LOG(ret == 0, ERR_OPERATION_FAILED,
            "FastAudioRendererSink::Pause failed!");
    }
    paused_ = true;
...
int32_t FastAudioRendererSinkInner::Resume(void)
{
    ...
    CHECK_AND_RETURN_RET_LOG(audioRender_ != nullptr, ERR_INVALID_HANDLE,
        "FastAudioRendererSink::Resume failed audioRender_ null");

    CHECK_AND_RETURN_RET_LOG(started_, ERR_OPERATION_FAILED,
        "FastAudioRendererSink::Resume invalid state!");

    if (paused_) {
        ret = audioRender_->Resume(audioRender_);
        CHECK_AND_RETURN_RET_LOG(ret == 0, ERR_OPERATION_FAILED,
            "FastAudioRendererSink::Resume failed!");
    }
    paused_ = false;
...
int32_t FastAudioRendererSinkInner::Reset(void)
{
    ...
    if (started_ && audioRender_ != nullptr) {
        ...
        CHECK_AND_RETURN_RET_LOG(ret == 0, ERR_OPERATION_FAILED,
            "FastAudioRendererSink::Reset failed!");
    }
...
int32_t FastAudioRendererSinkInner::Flush(void)
{
    ...
    if (started_ && audioRender_ != nullptr) {
        ret = audioRender_->Flush(audioRender_);
        CHECK_AND_RETURN_RET_LOG(ret == 0, ERR_OPERATION_FAILED,
            "FastAudioRendererSink::Flush failed!");
    }
