...
#ifdef DEBUG_DUMP_FILE
    AUDIO_INFO_LOG("Dump audio sink attr: [%{public}s]", printRemoteAttr(attr_).c_str());
...
int32_t RemoteFastAudioCapturerSource::CreateCapture(const struct AudioPort &capturePort)
{
    ...
    if (captureAttr.type == AUDIO_MMAP_NOIRQ) {
        InitAshmem(captureAttr); // The remote fast sink first start
    }
...
int32_t RemoteFastAudioCapturerSource::InitAshmem(const struct AudioSampleAttributes &attrs)
{
    ...
    // desc.memoryFd指内存首地址，当前memoryAddress没使用
    bufferFd_ = desc.memoryFd;
    // desc.totalBufferFrames指所有buf中的采样点数和。以48K-2声道-16bit，8个buf*5ms为例，共计48 * 40 = 1920个采样点
    bufferTotalFrameSize_ = desc.totalBufferFrames; // 1440 ~ 3840
    // desc.transferFrameSize指单个buf中的采样点数。以48K-2声道-16bit，8个buf*5ms为例，共计48 * 5 = 240个采样点
    ...
    AUDIO_INFO_LOG("%{public}s creat ashmem sink OK, ashmemLen %{public}d.", __func__, ashmemLen_);
    ...
    // lenPerRead_为每次读取的字节数，以48K-2声道-16bit，8个buf*5ms为例，等于2 * 2 * 48 * 5 = 960个字节
    AUDIO_INFO_LOG("%{public}s map ashmem sink OK, lenPerWrite %{public}d.", __func__, lenPerRead_);
    return SUCCESS;
}
...
int32_t RemoteFastAudioCapturerSource::CaptureFrame(char *frame, uint64_t requestBytes, uint64_t &replyBytes)
{
    ...
    auto data = ashmemSource_->ReadFromAshmem(lenPerRead_, readAshmemIndex_);
    CHECK_AND_RETURN_RET_LOG((data != nullptr), ERR_READ_FAILED,
        "%{public}s: Remote fast capturer read to ashmem fail.", __func__);
    readAshmemIndex_ += lenPerRead_;
    if (readAshmemIndex_ >= ashmemLen_) {
        readAshmemIndex_ = 0;
    }
    frame = const_cast<char *>(static_cast<const char *>(data));
