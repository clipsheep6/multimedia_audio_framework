...
{
    auto ret = AudioSystemManager::GetInstance()->SetDeviceActive(DeviceType::DEVICE_TYPE_MAX, true);
...
{
    auto isActive = AudioSystemManager::GetInstance()->IsDeviceActive(DeviceType::DEVICE_TYPE_MAX);
...
HWTEST(AudioManagerUnitTest, SetCallDeviceActive_001, TestSize.Level1)
{
    ...
    std::string address = "";
    auto ret = AudioSystemManager::GetInstance()->SetCallDeviceActive(DeviceType::DEVICE_TYPE_BLUETOOTH_SCO, true, address);
