    ...
    TestSize.Level1)
{
    ...
    for (int32_t i = 0; i < VALUE_HUNDRED; i++) {
        ...
        EXPECT_EQ(true, (audioRendererChangeInfos[0]->outputDeviceInfo.audioStreamInfo_.format >= SAMPLE_U8)
            && ((audioRendererChangeInfos[0]->outputDeviceInfo.audioStreamInfo_.format <= SAMPLE_F32LE)));
        audioRendererChangeInfos.clear();
    }
...
HWTEST(AudioStreamManagerUnitTest, Audio_Stream_Change_Listner_GetCurrentCapturerChangeDeviceInfos_001, TestSize.Level1)
{
    ...
    EXPECT_EQ(true, (audioCapturerChangeInfos[0]->inputDeviceInfo.audioStreamInfo_.format >= SAMPLE_U8)
        && ((audioCapturerChangeInfos[0]->inputDeviceInfo.audioStreamInfo_.format <= SAMPLE_F32LE)));
