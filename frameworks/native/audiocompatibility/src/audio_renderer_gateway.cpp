...
int32_t AudioRendererGateway::SetRendererCallback(const std::shared_ptr<AudioRendererCallback> &callback)
{
    ...
    AUDIO_DEBUG_LOG("callback audioStreamCallback_");
...
{
    AUDIO_DEBUG_LOG("NotifyEvent: Hint: %{public}d", interruptEvent.hintType);
    AUDIO_DEBUG_LOG("NotifyEvent: eventType: %{public}d", interruptEvent.eventType);
...
bool AudioInterruptCallbackGateway::HandleForceDucking(const InterruptEventInternal &interruptEvent)
{
    ...
    AUDIO_DEBUG_LOG("set duckVolumeDb(instance) %{pubic}f succeeded", duckInstanceVolume);
    return true;
}
...
void AudioInterruptCallbackGateway::HandleAndNotifyForcedEvent(const InterruptEventInternal &interruptEvent)
{
    ...
    AUDIO_DEBUG_LOG("HandleAndNotifyForcedEvent: Hint: %{public}d eventType: %{public}d",
...
void AudioStreamRenderCallback::OnStateChange(const State state, StateChangeCmdType cmdType)
{
    ...
    CHECK_AND_RETURN_LOG(cb != nullptr, "AudioStreamRenderCallback::OnStateChange cb == nullptr.");
