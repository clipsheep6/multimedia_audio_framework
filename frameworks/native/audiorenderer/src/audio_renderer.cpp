...
static constexpr int32_t WRITE_UNDERRUN_NUM = 3;
...
int32_t AudioRenderer::CheckMaxRendererInstances()
{
    ...
    if (audioRendererChangeInfos.size() > static_cast<size_t>(maxRendererInstances)) {
        ...
        bundleInfo = GetBundleInfoFromUid(mostAppUid);
        ...
        bean->Add("DUBIOUS_APP", bundleInfo.name);
    ...
    const AppInfo &appInfo)
{
    ...
    if (ret != SUCCESS) {
        AudioRenderer::SendRendererCreateError(rendererOptions.rendererInfo.streamUsage, appInfo, ret);
...
void AudioRenderer::SendRendererCreateError(const StreamUsage &sreamUsage,
    const AppInfo &appInfo, const int32_t &errorCode)
{
    AppExecFwk::BundleInfo bundleInfo = GetBundleInfoFromUid(appInfo.appUid);
    std::shared_ptr<Media::MediaMonitor::EventBean> bean = std::make_shared<Media::MediaMonitor::EventBean>(
...
bool AudioRendererPrivate::SwitchToTargetStream(IAudioStream::StreamClass targetClass)
{
    ...
    AppExecFwk::BundleInfo bundleInfo = GetBundleInfoFromUid(appInfo_.appUid);
    std::shared_ptr<Media::MediaMonitor::EventBean> bean = std::make_shared<Media::MediaMonitor::EventBean>(
        ...
        bean->Add("PIPE_TYPE_BEFORE_CHANGE", PIPE_TYPE_LOWLATENCY);
        bean->Add("PIPE_TYPE_AFTER_CHANGE", PIPE_TYPE_NORMAL);
