...
{
    AUDIO_INFO_LOG("lxj AudioRendererConcurrencyCallbackImpl ctor");
}

AudioRendererConcurrencyCallbackImpl::~AudioRendererConcurrencyCallbackImpl()
{
    AUDIO_INFO_LOG("lxj AudioRendererConcurrencyCallbackImpl dtor");
}
...
void AudioRendererConcurrencyCallbackImpl::OnConcedeStream()
{
    ...
    AUDIO_INFO_LOG("lxj Renderer OnConcedeStream");
...
int32_t AudioRendererPrivate::InitAudioConcurrencyCallback()
{
    ...
    AUDIO_INFO_LOG("lxj session: %{public}u", sessionID_);
...
{
    AUDIO_INFO_LOG("lxj set offload mode for session %{public}u", sessionID_);

    CHECK_AND_RETURN_RET_LOG(rendererInfo_.pipeType == PIPE_TYPE_NORMAL_OUT, ERR_CONCEDE_INCOMING_STREAM,
        "lxj session %{public}u pipe type is %{public}d, deny offload", sessionID_, rendererInfo_.pipeType);
    ...
    CHECK_AND_RETURN_RET_LOG(ret == SUCCESS, ERR_CONCEDE_INCOMING_STREAM,
        "lxj session %{public}u deny offload", sessionID_);
...
{
    AUDIO_INFO_LOG("lxj session %{public}u session unset offload", sessionID_);
    int32_t ret = audioStream_->UnsetOffloadMode();
    CHECK_AND_RETURN_RET_LOG(ret == SUCCESS, ret, "lxj unset offload failed");
    ret = AudioPolicyManager::GetInstance().MoveToNewPipe(sessionID_, PIPE_TYPE_NORMAL_OUT);
    CHECK_AND_RETURN_RET_LOG(ret == SUCCESS, ret, "lxj concede from offload failed");
    return SUCCESS;
}
