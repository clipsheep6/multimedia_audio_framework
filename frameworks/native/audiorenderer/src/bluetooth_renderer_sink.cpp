...
int32_t BluetoothRendererSink::Init(AudioSinkAttr &attr)
{
    ...
    MEDIA_INFO_LOG("call get all adapters");
    ret = audioManager_->GetAllAdapters(audioManager_, &descs, &size);
    MEDIA_INFO_LOG("get all adapters done");
    ...
    MEDIA_INFO_LOG("switch adapter done index: %{public}d", index);
    ...
    MEDIA_INFO_LOG("load adapter done");
    ...
    MEDIA_INFO_LOG("init ports done");

    if (CreateRender(audioPort) != 0) {
        MEDIA_ERR_LOG("Create render faied");
        return ERR_NOT_STARTED;
    }
    MEDIA_INFO_LOG("create renderer done");
