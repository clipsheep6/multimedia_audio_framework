...
TonePlayerPrivate::TonePlayerPrivate(const AudioRendererInfo &rendereInfo, float volume)
{
    ...
    mRendererOptions.streamInfo.samplingRate = SAMPLE_RATE_48000;
    ...
    // Kathir , Need to get sampling rate with respect to STREAM
    processSize_ = (samplingRate_ * 20) / 1000;
}
...
bool TonePlayerPrivate::loadTone(tone_type toneType)
{
    bool lResult = false;

    if (toneType >= TonePlayer::NUM_TONES) {
        return lResult;
    }
...
bool TonePlayerPrivate::startTone() 
{
    ...
    if (prepareWave()) {
        ...
        if (mState == TONE_PLAYER_STARTING) {
            ...
            lStatus = waitAudioCbkCond_.wait_for(lock, std::chrono::seconds(3));
            mutexLock_.lock(); // Newly added
    ...
    CHECK_AND_RETURN_RET_LOG(!lResult == true, lResult, "Tone start failed!!!, time %{public}d", (unsigned int)(tv.tv_sec));

    return lResult;
}

void TonePlayerPrivate::stopTone()
{
    struct timeval tv = {0};
    ...
    if (mState == TONE_PLAYER_PLAYING || mState == TONE_PLAYER_STARTING || mState == TONE_PLAYER_RESTARTING) {
        ...
        if ((startTime_.tv_sec != 0) && (clock_gettime(CLOCK_MONOTONIC, &stopTime) == 0)) {
            ...
            if ((sec + 1) > ((time_t)(INT_MAX / samplingRate_))) {
                ...
                maxSample = (unsigned int)(((int64_t)sec * samplingRate_) / 1000);
            }
    ...
    mutexLock_.unlock();
    std::unique_lock<std::mutex> lock(cbkCondLock_);
    status_t lStatus = waitAudioCbkCond_.wait_for(lock, std::chrono::seconds(3));
    mutexLock_.lock();
    if (lStatus != std::cv_status::timeout) {
...
bool TonePlayerPrivate::initAudioTrack() {
    processSize_ = (mRendererOptions.streamInfo.samplingRate * 20) /1000;
    pAudioRenderer_ = AudioRenderer::Create(mRendererOptions);
    if (pAudioRenderer_ == nullptr) {
        AUDIO_ERR_LOG("initAudioTrack: Renderer create failed");
        return false;
    }

    AUDIO_INFO_LOG("initAudioTrack: Playback renderer created");
    if (pAudioRenderer_->SetRenderMode(RENDER_MODE_CALLBACK)) {
        AUDIO_ERR_LOG("initAudioTrack: SetRenderMode failed");
        return false;
    }
    AUDIO_INFO_LOG("initAudioTrack: SetRenderMode Sucessful");
    if (pAudioRenderer_->SetRendererWriteCallback(shared_from_this())) {
        AUDIO_ERR_LOG("initAudioTrack: SetRendererWriteCallback failed");
        return false;
    }
    AUDIO_INFO_LOG("initAudioTrack: SetRendererWriteCallback Sucessful");
    if (pAudioRenderer_->SetRendererCallback(shared_from_this())) {
        AUDIO_ERR_LOG("initAudioTrack: SetRendererCallbackfailed");
        return false;
    }

    AUDIO_INFO_LOG("initAudioTrack: SetRendererCallback Sucessful");
    pAudioRenderer_->SetVolume(volume_);
    AUDIO_INFO_LOG("initAudioTrack: SetVolume Sucessful");
...
void TonePlayerPrivate::audioCallback()
{
    struct timeval tv = {0};
    BufferDesc bufDesc_ = {};
    ...
    unsigned int lNumSmp = lbufLength /sizeof(int16_t);
    std::shared_ptr<ToneInfo> lpToneDesc = pToneInfo_;

    AUDIO_INFO_LOG("Audio Callback, buflen: %{public}d  lNumSmp %{public}d  lbufLength %{public}d",
        bufDesc_.bufLength, lNumSmp, lbufLength);
    ...
    memset_(lpOut, bufDesc_.bufLength, 0, bufDesc_.bufLength);
 
    while (lNumSmp) {
        unsigned int lReqSmp = lNumSmp < processSize_*2 ? lNumSmp : processSize_;
        ...
        if (totalSample > nextSegSample) {
            ...
            if (lpToneDesc->segments[currSegment].duration == 0) {
                ...
                if (++currCount <= lpToneDesc->repeatCnt) {
                    ...
                    AUDIO_INFO_LOG("New segment %{public}d, Next Time: %lld", currSegment,
                        ((long long)(nextSegSample)*1000)/samplingRate_);
                } else {
                    lGenSmp = 0;
                    gettimeofday(&tv, NULL);
            ...
            nextSegSample
                    += (lpToneDesc->segments[currSegment].duration * samplingRate_) / 1000;

            AUDIO_INFO_LOG("AudioCallback, nextSegSample %{public}d", nextSegSample);
        } else {
        ...
        AUDIO_INFO_LOG("AudioCallback, lGenSmp %{public}d", lGenSmp);
        if (lGenSmp) {
...
audioCallback_EndLoop:
    ...
    // comment mutexLock_.unlock();
    AUDIO_INFO_LOG("Exiting the AudioCallback %{public}d, mState %{public}d ", bufDesc_.dataLength, mState);
...
bool TonePlayerPrivate::prepareWave()
{
    ...
    if (durationMs_ == -1) {
        ...
        if (durationMs_ > (int)(TONEINFO_INF / samplingRate_)) {
            maxSample = (durationMs_ / 1000) * samplingRate_;
        } else {
            maxSample = (durationMs_ * samplingRate_) / 1000;
        }
    ...
    if (pToneInfo_->segments[0].duration == TONEINFO_INF) {
        ...
        nextSegSample = (pToneInfo_->segments[0].duration * samplingRate_) / 1000;
    }
