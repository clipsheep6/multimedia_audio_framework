...
bool TonePlayerPrivate::LoadEventStateHandler()
{
    ...
    if (audioRenderer_ != nullptr) {
        AUDIO_INFO_LOG("TonePlayerPrivate::LoadEventStateHandler audioRenderer_ is not null");
        StopTone();
        AUDIO_INFO_LOG("TonePlayerPrivate::LoadEventStateHandler StopTone finish");
        audioRenderer_->Clear();
        tonePlayerState_ = TONE_PLAYER_IDLE;
        AUDIO_INFO_LOG("TonePlayerPrivate::LoadEventStateHandler playState: %{public}d", tonePlayerState_);
        if (toneDataGenLoop_ && toneDataGenLoop_->joinable()) {
            toneDataGenLoop_->join();
            AUDIO_INFO_LOG("TonePlayerPrivate::LoadEventStateHandler toneDataGenLoop_ join finish");
        }
        audioRenderer_ = nullptr;
    }
    if (tonePlayerState_ == TONE_PLAYER_IDLE) {
        ...
        if ((audioRenderer_ == nullptr) && (!InitAudioRenderer())) {
            AUDIO_INFO_LOG("TonePlayerPrivate::LoadEventStateHandler InitAudioRenderer false");
            return false;
        }
    ...
    if (InitToneWaveInfo() == false) {
        ...
        tonePlayerState_ = TONE_PLAYER_IDLE;
        AUDIO_INFO_LOG("TonePlayerPrivate::LoadEventStateHandler InitToneWaveInfo false");
        return false;
    }
    ...
    AUDIO_INFO_LOG("TonePlayerPrivate::LoadEventStateHandler end return true");
    return true;
}

bool TonePlayerPrivate::PlayEventStateHandler()
{
    AUDIO_INFO_LOG("TonePlayerPrivate::PlayEventStateHandler start");
    status_t retStatus;
    mutexLock_.unlock();
    ...
    AUDIO_INFO_LOG("TonePlayerPrivate::PlayEventStateHandler return true");
    return true;
}

bool TonePlayerPrivate::TonePlayerStateHandler(int16_t event)
{
    AUDIO_INFO_LOG("TonePlayerPrivate::TonePlayerStateHandler start event: %{public}d", event);
    switch (event) {
    ...
    AUDIO_INFO_LOG("TonePlayerPrivate::TonePlayerStateHandler end");
    return true;
}
...
{
    AUDIO_INFO_LOG("TonePlayerPrivate::LoadTone type: %{public}d", toneType);
    bool lRet = false;
    AUDIO_INFO_LOG("LoadTone tonePlayerState_ %{public}d", tonePlayerState_);
    if (toneType >= NUM_TONES) {
        AUDIO_INFO_LOG("TonePlayerPrivate::LoadTone ---1");
        return lRet;
    }
    mutexLock_.lock();
    if (tonePlayerState_ != TONE_PLAYER_IDLE && tonePlayerState_ != TONE_PLAYER_INIT) {
        mutexLock_.unlock();
        AUDIO_INFO_LOG("TonePlayerPrivate::LoadTone ---2");
        return lRet;
    }

    if (std::find(supportedTones_.begin(), supportedTones_.end(), (int32_t)toneType) == supportedTones_.end()) {
        mutexLock_.unlock();
        AUDIO_INFO_LOG("TonePlayerPrivate::LoadTone ---3");
        return lRet;
    }
    // Get descriptor for requested tone
    initialToneInfo_ = AudioPolicyManager::GetInstance().GetToneConfig(toneType);
    AUDIO_INFO_LOG("TonePlayerPrivate::LoadTone ---4");
#ifdef DUMPFILE
    AUDIO_INFO_LOG("initialToneInfo_, repeatCnt: %{public}d, repeatSegment: %{public}d, segmentCnt: %{public}d",
        initialToneInfo_->repeatCnt, initialToneInfo_->repeatSegment, initialToneInfo_->segmentCnt);
    AUDIO_INFO_LOG("----------------------------------segments message.---------------------------------------");
    for (int i = 0; i < 13; i++) {
        if (initialToneInfo_->segments[i].duration == 0)
	    break;
        AUDIO_INFO_LOG("initialToneInfo_---segms[%{public}d], duration: %{public}d, loopCnt: %{public}d, loopIndx: %{public}d",
            i, initialToneInfo_->segments[i].duration, initialToneInfo_->segments[i].loopCnt,
            initialToneInfo_->segments[i].loopIndx);
        AUDIO_INFO_LOG("----------------------------------wave message.---------------------------------------");
        for (int j = 0; j < 4; j++) {
	    if (initialToneInfo_->segments[i].waveFreq[j] == 0) break;
            AUDIO_INFO_LOG("waveFreq: %{public}d", initialToneInfo_->segments[i].waveFreq[j]);
        }
        AUDIO_INFO_LOG("----------------------------------wave message.---------------------------------------");
    }
    AUDIO_INFO_LOG("----------------------------------segments message.---------------------------------------");
...
#endif // DUMPFILE
    AUDIO_INFO_LOG("TonePlayerPrivate::LoadTone return true");
    return true;
}

bool TonePlayerPrivate::StartTone()
{
    AUDIO_INFO_LOG("TonePlayerPrivate::StartTone");
    bool retVal = false;
    AUDIO_INFO_LOG("STARTTONE tonePlayerState_ %{public}d", tonePlayerState_);
    ...
    if (tonePlayerState_ == TONE_PLAYER_IDLE || tonePlayerState_ == TONE_PLAYER_INIT) {
        if (LoadEventStateHandler() == false) {
            mutexLock_.unlock();
            AUDIO_INFO_LOG("STARTTONE end tonePlayerState_ %{public}d", tonePlayerState_);
            return false;
        }
    }
    retVal = TonePlayerStateHandler(PLAYER_EVENT_PLAY);
    mutexLock_.unlock();
    AUDIO_INFO_LOG("TonePlayerPrivate::StartTone end");
    return retVal;
}

bool TonePlayerPrivate::Release()
{
    AUDIO_INFO_LOG("TonePlayerPrivate::Release");
    if (audioRenderer_ != nullptr) {
        StopTone();
        audioRenderer_->Clear();
        tonePlayerState_ = TONE_PLAYER_IDLE;
    ...
    audioRenderer_ = nullptr;
    AUDIO_INFO_LOG("TonePlayerPrivate::StartTone end return true");
    return true;
}

bool TonePlayerPrivate::StopTone()
{
    AUDIO_INFO_LOG("TonePlayerPrivate::StopTone start tonePlayerState_: %{public}d", tonePlayerState_);
    bool retVal = true;
    ...
    if (tonePlayerState_ == TONE_PLAYER_IDLE  || tonePlayerState_ == TONE_PLAYER_INIT) {
        ...
        mutexLock_.unlock();
        AUDIO_INFO_LOG("TonePlayerPrivate::StopTone end tonePlayerState_: %{public}d", tonePlayerState_);
        return retVal;
    }
...
#endif // DUMPFILE
    AUDIO_INFO_LOG("TonePlayerPrivate::StopTone end");
    return retVal;
}

bool TonePlayerPrivate::StopEventStateHandler()
{
    AUDIO_INFO_LOG("TonePlayerPrivate::StopEventStateHandler start tonePlayerState_: %{public}d", tonePlayerState_);
    ...
    AUDIO_INFO_LOG("TonePlayerPrivate::StopEventStateHandler return true tonePlayerState_: %{public}d", tonePlayerState_);
    return true;
}
...
{
    AUDIO_ERR_LOG("initAudioRenderer start");
    ...
    AUDIO_ERR_LOG("initAudioRenderer end");
    return true;
}
...
{
    AUDIO_INFO_LOG("CheckToneStopped start");
    ...
    if (toneDesc->segments[currSegment_].duration == 0 ||
        ...
        AUDIO_INFO_LOG("CheckToneStopped return true tonePlayerState_ %{public}d", tonePlayerState_);
        return true;
    }
    AUDIO_INFO_LOG("CheckToneStopped return false tonePlayerState_ %{public}d", tonePlayerState_);
    return false;
}

bool TonePlayerPrivate::CheckToneStarted(uint32_t reqSample, int8_t *audioBuffer)
{
    AUDIO_INFO_LOG("GTonePlayerPrivate::CheckToneStarted start playState: %{public}d ", tonePlayerState_);
    std::shared_ptr<ToneInfo> toneDesc = toneInfo_;
    if (tonePlayerState_ == TONE_PLAYER_STARTING) {
        ...
        AUDIO_INFO_LOG("CheckToneStarted GenerateStartWave to currseg: %{public}d playState: %{public}d ", currSegment_, tonePlayerState_);
        return true;
    }
    AUDIO_INFO_LOG("CheckToneStarted GenerateStartWave return false playState: %{public}d ", tonePlayerState_);
    return false;
}

void TonePlayerPrivate::GetCurrentSegmentUpdated(std::shared_ptr<ToneInfo> toneDesc)
{
    AUDIO_INFO_LOG("GTonePlayerPrivate::GetCurrentSegmentUpdated start");
...
{
    AUDIO_INFO_LOG("GTonePlayerPrivate::CheckToneContinuity start");
    bool retVal = false;
...
{
    AUDIO_INFO_LOG("ContinueToneplay start");
    std::shared_ptr<ToneInfo> toneDesc = toneInfo_;
    if (tonePlayerState_ != TONE_PLAYER_PLAYING) {
        return false;
    }
    
    AUDIO_INFO_LOG("ContinueToneplay, totalSample_: %{public}d, nextSegSample_: %{public}d, currSegment_: %{public}d.",
        totalSample_, nextSegSample_, currSegment_);
    if (totalSample_ <= nextSegSample_) {
        AUDIO_INFO_LOG("ContinueToneplay Continue Playing Tone");
        if (toneDesc->segments[currSegment_].duration != 0) {
            GetSamples(toneDesc->segments[currSegment_].waveFreq, audioBuffer, reqSample);
        }
        AUDIO_INFO_LOG("ContinueToneplay return true nextSegSample_ : %{public}d", nextSegSample_);
        return true;
    }
...
{
    AUDIO_INFO_LOG("AudioToneRendererCallback start");
...
bool TonePlayerPrivate::AudioToneSequenceGen(BufferDesc &bufDesc)
{
    ...
    AUDIO_INFO_LOG("AudioToneSequenceGen, =========================totalBufAvailable : %{public}d tonePlayerState_: %{public}d", totalBufAvailable, tonePlayerState_);
    while (totalBufAvailable) {
        ...
        AUDIO_INFO_LOG("before checkTone ------------------0 tonePlayerState_: %{public}d",tonePlayerState_);
        if (CheckToneStopped()) {
            AUDIO_INFO_LOG("--------------------------------------1 tonePlayerState_: %{public}d",tonePlayerState_);
            ...
            if (tonePlayerState_ == TONE_PLAYER_STOPPED) {
                ...
                audioRenderer_->Stop();
                AUDIO_INFO_LOG("AudioToneSequenceGen return false stop");
                return false;
            }
        } else if (CheckToneStarted(reqSamples, audioBuffer)) {
            AUDIO_INFO_LOG("--------------------------------------2 tonePlayerState_: %{public}d",tonePlayerState_);
            bufDesc.dataLength += reqSamples * sizeof(int16_t);
            lSignal = true;
        } else {
            AUDIO_INFO_LOG("--------------------------------------3 tonePlayerState_: %{public}d",tonePlayerState_);
    ...
    AUDIO_INFO_LOG("AudioToneSequenceGen return true");
    return true;
}

void TonePlayerPrivate::AudioToneDataThreadFunc()
{
    AUDIO_INFO_LOG("TonePlayerPrivate::AudioToneDataThreadFunc start");
    while (true) {
        BufferDesc bufDesc = {};
        ...
        if (bufDesc.bufLength == 0) {
            AUDIO_INFO_LOG("AudioToneDataThread return, buflen: %{public}zu ", bufDesc.bufLength);
            return;
        }
        ...
        if (AudioToneSequenceGen(bufDesc) == false) {
            AUDIO_INFO_LOG("AudioToneDataThread, AudioToneSequenceGen return false, bufDesclen : %{public}zu ", bufDesc.bufLength);
            return;
        }
        ...
        if (bufDesc.dataLength) {
            ...
            AUDIO_INFO_LOG("AudioToneDataThreadFunc ------fwrite %{public}zu,tonePlayerState_ %{public}d ",
            writeResult, tonePlayerState_);
            ...
            if (toneDataState_ == TONE_DATA_LOADING) {
                ...
                AUDIO_INFO_LOG("wait_for CMAXWAIT");
                waitToneDataCond_.wait_for(lock, std::chrono::seconds(CMAXWAIT));
            }
        ...
        AUDIO_INFO_LOG("AudioToneDataThreadFunc end %{public}zu,tonePlayerState_ %{public}d ",
            bufDesc.dataLength, tonePlayerState_); 
    }
}
...
{
    AUDIO_INFO_LOG("onePlayerPrivate::GetSamples, Entered");
    uint32_t index;
    uint8_t* data;
    uint16_t freqVal;
    ...
    AUDIO_INFO_LOG("onePlayerPrivate::GetSamples end, sampleCount_ %{public}d", sampleCount_);
    return 0;
}
