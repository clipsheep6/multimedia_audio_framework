...
private:
    ...
    static constexpr int16_t GEN_AMP = 32000;  // amplitude of generator
    static constexpr int16_t SHIFT_Q14 = 14;  // shift for Q14
    static constexpr int16_t SHIFT_Q15 = 15;  // shift for Q15
    ...
    enum wav_phase {
        WAVE_PHASE_INIT,  // Start/restart wave from phase 0
        WAVE_PHASE_CONT,  // Continue wave from current phase
        WAVE_PHASE_STOP  // Stop wave on zero crossing
    };
    ...
    uint32_t totalSample_;  // Total number of audio samples played (gives current time)
    ...
    std::map <uint16_t, WaveInfo *> waveGenInfos_; //list of all active wave information

    std::string mOpPackageName;
    bool InitAudioRenderer();
    void AudioToneRendererCallback();
    bool InitToneWaveInfo();
    bool GenerateWaveInfos(uint16_t frequency, uint16_t lNumOfWaves);
    bool GenerateEndWave(int16_t frequency, int16_t *outBuffer, uint16_t sampleCnt);
    bool GenerateWave(int16_t frequency, int16_t *outBuffer, uint16_t sampleCnt);
    bool GenerateStartWave(int16_t frequency, int16_t *outBuffer, uint16_t sampleCnt);
    bool ContinueToneplay(uint32_t sampleCnt, int16_t * lpAudioBuf);
    bool CheckToneStarted(uint32_t sampleCnt, int16_t * lpAudioBuf);
    bool CheckToneStopped(uint32_t sampleCnt, int16_t * lpAudioBuf);
    bool CheckToneContinuity ();
    uint16_t NumOfWaves(uint16_t segmentIdx);
    void clearWaveGenInfos();
};
} // namespace AudioStandard
} // namespace OHOS
