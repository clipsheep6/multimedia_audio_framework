    ...
    std::string sinkName)
{
    ...
    CHECK_AND_RETURN_RET_LOG(mContext != nullptr, ERROR, "SetVolume mContext is nullptr");
    pa_threaded_mainloop_lock(mMainLoop);
    ...
    std::string sourceName)
{
    ...
    if (mContext == nullptr) {
        AUDIO_ERR_LOG("SetVolume mContext is nullptr");
        return ERROR;
    }
    pa_threaded_mainloop_lock(mMainLoop);
    pa_operation *operation = nullptr;
...
int32_t PulseAudioServiceAdapterImpl::SetSourceOutputMute(int32_t uid, bool setMute)
{
    ...
    AUDIO_DEBUG_LOG("set %{public}d %{public}s", streamSet, (setMute ? "mute" : "unmuted"));
    return streamSet;
}
...
{
    if (mContext != nullptr) {
        AUDIO_DEBUG_LOG("disconnect context!");
        pa_context_disconnect(mContext);
        /* Make sure we don't get any further callbacks */
        pa_context_set_state_callback(mContext, nullptr, nullptr);
        pa_context_set_subscribe_callback(mContext, nullptr, nullptr);
    ...
    void *userdata)
{
    ...
    AUDIO_INFO_LOG("GetSinkInputInfoVolumeCb enter.");
    if (eol < 0) {
        delete userData;
    ...
    void *userdata)
{
    ...
    CHECK_AND_RETURN_LOG(eol >= 0, "PaGetAllSinkInputsCb Failed to get sink input information: %{public}s",
        pa_strerror(pa_context_errno(c)));
    ...
    CHECK_AND_RETURN_LOG(i->proplist != nullptr,
       "PaGetAllSinkInputsCb Invalid Proplist for sink input (%{public}d).", i->index);
    ...
    switch (t & PA_SUBSCRIPTION_EVENT_FACILITY_MASK) {
        ...
        case PA_SUBSCRIPTION_EVENT_SINK_INPUT:
            if ((t & PA_SUBSCRIPTION_EVENT_TYPE_MASK) == PA_SUBSCRIPTION_EVENT_NEW) {
                ...
                AUDIO_DEBUG_LOG("sessionID: %{public}d  removed", sessionID);
                g_audioServiceAdapterCallback->OnSessionRemoved(sessionID);
            }
            break;
