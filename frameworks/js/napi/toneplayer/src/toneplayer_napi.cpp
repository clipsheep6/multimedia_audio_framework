    ...
    const std::string name, int32_t enumValue)
{
    ...
    if (status == napi_ok) {
        HiLog::Error(LABEL, "TonePlayerNapi::ParseRendererInfo() return status is ok11111111111");
        status = napi_set_named_property(env, object, name.c_str(), enumNapiValue);
    }
    HiLog::Error(LABEL, "TonePlayerNapi::ParseRendererInfo() return status is not ok");
    return status;
}
...
napi_value TonePlayerNapi::CreateToneTypeObject(napi_env env)
{
    ...
    if (status == napi_ok) {
        ...
        if (status == napi_ok) {
            ...
            if (status == napi_ok) {
	        HiLog::Error(LABEL, "TonePlayerNapi::ParseRendererInfo() return status is ok");
                return result;
            }
        }
    }
    ...
    HiLog::Error(LABEL, "TonePlayerNapi::ParseRendererInfo() return");
    return result;
}
...
bool TonePlayerNapi::ParseRendererInfo(napi_env env, napi_value root, AudioRendererInfo *rendererInfo)
{
    ...
    HiLog::Error(LABEL, "TonePlayerNapi::ParseRendererInfo() return true");
    return true;
}
...
napi_value TonePlayerNapi::CreateTonePlayerWrapper(napi_env env, unique_ptr<AudioRendererInfo> &rendererInfo)
{
    ...
    if (rendererInfo != nullptr) {
        ...
        if (status == napi_ok) {
            ...
            if (status == napi_ok) {
	        HiLog::Error(LABEL, "TonePlayerNapi::CreateTonePlayerWrapper() return status is ok");
                return result;
            }
        }
    ...
    napi_get_undefined(env, &result);
    HiLog::Error(LABEL, "TonePlayerNapi::CreateTonePlayerWrapper() return");
    ...
    const napi_value &valueParam)
{
    ...
    if (asyncContext->deferred) {
        if (!asyncContext->status) {
            ...
            // HiLog::Error(LABEL, "CommonCallbackRoutine return : , %{public}s", result[PARAM1]);

        } else {
            napi_reject_deferred(env, asyncContext->deferred, result[PARAM0]);
            // HiLog::Error(LABEL, "CommonCallbackRoutine reject return : , %{public}s", result[PARAM0]);
        }
    } else {
        napi_value callback = nullptr;
        napi_get_reference_value(env, asyncContext->callbackRef, &callback);
        napi_call_function(env, nullptr, callback, ARGS_TWO, result, &retVal);
        // HiLog::Error(LABEL, "CommonCallbackRoutine callback return : , %{public}s", &retVal);
...
void TonePlayerNapi::GetTonePlayerAsyncCallbackComplete(napi_env env, napi_status status, void *data)
{
    ...
    if (asyncContext != nullptr) {
        if (!asyncContext->status) {
            ...
            // HiLog::Error(LABEL, "CreateTonePlayerWrapper return : , %{public}d", valueParam);
        }
        CommonCallbackRoutine(env, asyncContext, valueParam);
        // HiLog::Error(LABEL, "CommonCallbackRoutine asyncContext return : , %{public}s", asyncContext->rendererInfo);
    } else {
...
napi_value TonePlayerNapi::CreateTonePlayer(napi_env env, napi_callback_info info)
{
    ...
    if (status != napi_ok) {
        HiLog::Error(LABEL, "TonePlayerNapi::GetAbilityContext() status is not ok");
        result = nullptr;
    } else {
...
static shared_ptr<AbilityRuntime::Context> GetAbilityContext(napi_env env)
{
    ...
    HiLog::Error(LABEL, "TonePlayerNapi::GetAbilityContext()! return");
    return faContext;
}
...
napi_value TonePlayerNapi::Construct(napi_env env, napi_callback_info info)
{
    ...
    if (abilityContext != nullptr) {
        ...
        if (!tonePlayerNapi->tonePlayer_) {
           HiLog::Info(LABEL, "param 2 tonePlayerNapi->tonePlayer_ is null ");
        }
        HiLog::Info(LABEL, "param 2 tonePlayerNapi->tonePlayer_ is not null ");
    } else {
        tonePlayerNapi->tonePlayer_ = TonePlayer::Create(rendererInfo);
        if (!tonePlayerNapi->tonePlayer_) {
           HiLog::Info(LABEL, "tonePlayerNapi->tonePlayer_ is null ");
        }
        HiLog::Info(LABEL, "tonePlayerNapi->tonePlayer_ is not null ");
        // HiLog::Info(LABEL, "tonePlayerNapi->tonePlayer_ : %{public}s",tonePlayerNapi->tonePlayer_);
    }
    ...
    HiLog::Error(LABEL, "TonePlayerNapi::Construct()! return");
    return result;
}
...
napi_value TonePlayerNapi::Load(napi_env env, napi_callback_info info)
{
    ...
    if (status == napi_ok && asyncContext->objectInfo != nullptr) {
        ...
        HiLog::Info(LABEL, "TonePlayerNapi::Load() napi_create_async_work ret ---1");   
        if (status != napi_ok) {
            HiLog::Info(LABEL, "TonePlayerNapi::Load() napi_create_async_work ret ---2"); 
            result = nullptr;
        } else {
            HiLog::Info(LABEL, "TonePlayerNapi::Load() napi_create_async_work ret ---3"); 
            status = napi_queue_async_work(env, asyncContext->work);
            if (status == napi_ok) {
                HiLog::Info(LABEL, "TonePlayerNapi::Load() napi_create_async_work ret ---4"); 
                asyncContext.release();
            } else {
                HiLog::Info(LABEL, "TonePlayerNapi::Load() napi_create_async_work ret ---5"); 
                result = nullptr;
            }
        }
    }
    HiLog::Error(LABEL, "TonePlayerNapi::Load()! return");
    return result;
}
...
napi_value TonePlayerNapi::Start(napi_env env, napi_callback_info info)
{
    ...
    HiLog::Error(LABEL, "TonePlayerNapi::Start()! return");
    return result;
}
...
napi_value TonePlayerNapi::Stop(napi_env env, napi_callback_info info)
{
    ...
    HiLog::Error(LABEL, "TonePlayerNapi::Stop()! return");
    return result;
}
...
napi_value TonePlayerNapi::Release(napi_env env, napi_callback_info info)
{
    ...
    HiLog::Error(LABEL, "TonePlayerNapi::Release()! return");
    return result;
}
...
void TonePlayerNapi::VoidAsyncCallbackComplete(napi_env env, napi_status status, void *data)
{
    ...
    if (asyncContext != nullptr) {
        ...
        CommonCallbackRoutine(env, asyncContext, valueParam);
	HiLog::Error(LABEL, "ERROR: VoidAsyncCallbackComplete* is not Null!");
    } else {
...
napi_value TonePlayerNapi::Init(napi_env env, napi_value exports)
{
    ...
    if (status != napi_ok) {
        HiLog::Error(LABEL, "TonePlayerNapi::Init()! status not ok return");
        return result;
    }
