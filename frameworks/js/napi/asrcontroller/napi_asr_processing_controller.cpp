...
namespace OHOS {
    namespace AudioStandard {
        ...
        {
            AUDIO_INFO_LOG("---CreateAudioManagerWrapper");
            napi_status status;
            napi_value result = nullptr;
            napi_value constructor;
            ...
            AUDIO_INFO_LOG("---CreateAudioManagerWrapper exit");
            return result;
        ...
        {
            AUDIO_INFO_LOG("createAsrProcessingController");
            napi_status status;
            size_t argCount = PARAM0;
        ...
        {
            AUDIO_INFO_LOG("asr_ssl_log SetAsrAecMode");
            napi_value result = nullptr;
            ...
            //AUDIO_INFO_LOG("-----------param0 : %{public}d", *(int32_t*)argv[0]);
            int32_t asrAecMode=0;
            NapiParamUtils::GetValueInt32(env, asrAecMode, argv[PARAM0]);
            AUDIO_INFO_LOG("-----------param0 : %{public}d", asrAecMode);
            CHECK_AND_RETURN_RET_LOG(napiAsrController != nullptr, result, "napiAudioManager is nullptr");
            CHECK_AND_RETURN_RET_LOG(napiAsrController->audioMngr_ != nullptr, result, "audioMngr_ is nullptr");
            napiAsrController->audioMngr_->SetAsrAecMode(static_cast<AsrAecMode>(asrAecMode));
            // 处理返回值
            //switch () {} todo
            AUDIO_INFO_LOG("asr_aec_log SetAsrAecMode napi done.");
            return result;
        }
