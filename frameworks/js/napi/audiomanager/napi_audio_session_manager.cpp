...
napi_value NapiAudioSessionMgr::Init(napi_env env, napi_value exports)
{
    ...
    napi_property_descriptor audio_session_mgr_properties[] = {
        ...
        DECLARE_NAPI_FUNCTION("activeAudioSession", ActiveAudioSession),
        DECLARE_NAPI_FUNCTION("DeactiveAudioSession", deactiveAudioSession),
        DECLARE_NAPI_FUNCTION("isAudioSessionActived", IsAudioSessionActived),
    };
...
napi_value NapiAudioSessionMgr::ActivateAudioSession(napi_env env, napi_callback_info info)
{
    ...
    auto executor = [context]() {
        ...
        context->intValue = napiSessionMgr->audioSessionMngr_->ActiveAudioSession(context->audioSessionStrategy);
...
napi_value NapiAudioSessionMgr::DectivateAudioSession(napi_env env, napi_callback_info info)
{
    auto context = std::make_shared<AudioSessionMgrAsyncContext>();
    if (context == nullptr) {
        AUDIO_ERR_LOG("DectivateAudioSession failed : no memory");
        NapiAudioError::ThrowError(env, "DectivateAudioSession failed : no memory", NAPI_ERR_NO_MEMORY);
        return NapiParamUtils::GetUndefinedValue(env);
    }
    context->GetCbInfo(env, info, inputParser);
...
napi_value NapiAudioSessionMgr::IsAudioSessionActived(napi_env env, napi_callback_info info)
{
    napi_value result = nullptr;
    auto context = std::make_shared<AudioSessionMgrAsyncContext>();
    if (context == nullptr) {
        AUDIO_ERR_LOG("IsAudioSessionActived failed : no memory");
        NapiAudioError::ThrowError(env, "IsAudioSessionActived failed : no memory", NAPI_ERR_NO_MEMORY);
        return NapiParamUtils::GetUndefinedValue(env);
    }
    context->GetCbInfo(env, info, inputParser);
    ...
    ObjectRefMap objectGuard(obj);
    auto *napiSessionMgr = objectGuard.GetPtr();
    context->isActive = napiSessionMgr->audioSessionMngr_->IsAudioSessionActived();
...
napi_value NapiAudioSessionMgr::On(napi_env env, napi_callback_info info)
{
    ...
    CHECK_AND_RETURN_RET_LOG(eventType == napi_function, NapiAudioError::ThrowErrorAndReturn(env, NAPI_ERR_INPUT_INVALID,
        "incorrect parameter types: The type of eventType must be function"), "handler is invalid");
...
void NapiAudioSessionMgr::UnregisterCallback(napi_env env, napi_value jsThis)
{
    ...
    If(napiSessionMgr->audioSessionCallbackNapi_ != nullptr) {
        napiSessionMgr->audioSessionCallbackNapi_.rest();
        napiSessionMgr->audioSessionCallbackNapi_ = nullptr;
    }
    AUDIO_ERR_LOG("Unset AudioSessionCallback Success");
}

void NapiAudioSessionMgr::UnregisterCallbackCarryParam(napi_env env, napi_value jsThis)
{
    AUDIO_INFO_LOG("UnregisterCallback");
    NapiAudioSessionMgr *napiSessionMgr = nullptr;
    napi_status status = napi_unwrap(env, jsThis, reinterpret_cast<void **>(&napiSessionMgr));
