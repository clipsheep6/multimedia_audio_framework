...
static void CommonCallbackRoutine(napi_env env,
                                  AudioRoutingManagerAsyncContext *&asyncContext, const napi_value &valueParam)
{
...
static void ParseAudioDeviceDescriptorVector(napi_env env,
                                             napi_value root,
...
{
    AUDIO_INFO_LOG("zhanhang routing test getdevcies");
    ...
    AUDIO_INFO_LOG("zhanhang routing test getdevcie 2s");
...
{
    AUDIO_INFO_LOG("zhanhang selectOutp");
    napi_status status;
    const int32_t refCount = 1;
    napi_value result = nullptr;
    size_t argc = ARGS_TWO;
