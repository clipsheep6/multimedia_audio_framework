    ...
    sptr<AudioDeviceDescriptor> &selectedAudioDevice, bool &argTransFlag)
{
    ...
    if (napi_get_named_property(env, root, "networkName", &tempValue) == napi_ok) {
        napi_get_value_string_utf8(env, tempValue, buffer, SIZE, &res);
        selectedAudioDevice->networkName_ = std::string(buffer);
    }
    ...
    napi_value valueParam)
{
    ...
    for (size_t i = 0; i < size; i++) {
        if (deviceDescriptors[i] != nullptr) {
            ...
            SetValueString(env, "networkName", static_cast<std::string>(
                deviceDescriptors[i]->networkName_), valueParam);
