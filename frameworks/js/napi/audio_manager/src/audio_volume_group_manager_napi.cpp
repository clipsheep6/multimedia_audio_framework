    ...
    if (status == napi_ok && asyncContext->objectInfo != nullptr) {
        ...
        status = napi_create_async_work(
            ...
            [](napi_env env, void *data) {
                ...
                AUDIO_INFO_LOG("IsVolumeUnadjustable %{public}d", context->isVolumeUnadjustable);
...
void GetArgvForAdjustVolume(napi_env env, napi_callback_info info,
...
napi_value AudioVolumeGroupManagerNapi::AdjustVolumeByStep(napi_env env, napi_callback_info info)
{
    ...
    GetArgvForAdjustVolume(env, info, asyncContext);
...
void GetArgvForAdjustSystemVolume(napi_env env, napi_callback_info info,
    unique_ptr<AudioVolumeGroupManagerAsyncContext> &asyncContext)
{
    const int32_t refCount = 1;
    GET_PARAMS(env, info, ARGS_THREE);
...
napi_value AudioVolumeGroupManagerNapi::AdjustSystemVolumeByStep(napi_env env, napi_callback_info info)
{
    ...
    GetArgvForAdjustSystemVolume(env, info, asyncContext);
...
void GetArgvForSystemVolumeInDb(napi_env env, napi_callback_info info,
    unique_ptr<AudioVolumeGroupManagerAsyncContext> &asyncContext)
{
    const int32_t refCount = 1;
...
napi_value AudioVolumeGroupManagerNapi::GetSystemVolumeInDb(napi_env env, napi_callback_info info)
{
    ...
    status = napi_unwrap(env, thisVar, reinterpret_cast<void**>(&asyncContext->objectInfo));
    if (status == napi_ok && asyncContext->objectInfo != nullptr) {
        GetArgvForSystemVolumeInDb(env, info, asyncContext);
        if (asyncContext->callbackRef == nullptr) {
            napi_create_promise(env, &asyncContext->deferred, &result);
        } else {
            napi_get_undefined(env, &result);
