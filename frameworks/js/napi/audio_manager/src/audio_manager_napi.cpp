...
{
    AUDIO_DEBUG_LOG("JsObjectToInt: start");
...
{
    AUDIO_DEBUG_LOG("JsObjectToBool: start");
    napi_value tempValue = nullptr;
    napi_status status = napi_get_named_property(env, object, fieldStr.c_str(), &tempValue);
    if (status != napi_ok) {
...
{
    AUDIO_DEBUG_LOG("JsObjToAudioInterrupt: start");
    int32_t propValue = -1;
    ...
    AUDIO_DEBUG_LOG("JsObjToAudioInterrupt: contentType %{public}d,streamUsage %{public}d", audioInterrupt.contentType,audioInterrupt.streamUsage );
    return status;
}
...
napi_value AudioManagerNapi::On(napi_env env, napi_callback_info info)
{
    ...
    if (argCount == minArgCount) {
        ...
        AUDIO_DEBUG_LOG("AudioManagerNapi::Off SetAudioManagerInterruptCallback and RequestAudioFocus is successful");
    }
...
napi_value AudioManagerNapi::Off(napi_env env, napi_callback_info info)
{
    ...
    if (!callbackName.compare(INTERRUPT_CALLBACK_NAME) && argCount > ARGS_ONE) {
        ...
        AUDIO_DEBUG_LOG("AudioManagerNapi::Off Abandon Focus and UnSetAudioInterruptCallback successful");
    } else if (!callbackName.compare(DEVICE_CHANGE_CALLBACK_NAME)) {
        ...
        AUDIO_DEBUG_LOG("AudioManagerNapi::Off UnsetDeviceChangeCallback Success");
    }
    return undefinedResult;
}
