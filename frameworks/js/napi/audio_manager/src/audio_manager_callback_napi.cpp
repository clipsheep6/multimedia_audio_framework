...
static void NativeDeviceChangeActionToJsObj(const napi_env& env, napi_value& jsObj, const DeviceChangeAction &action)
{
    ...
    for (size_t i = 0; i < size; i++) {
        if (action.deviceDescriptors[i] != nullptr) {
            ...
            SetValueInt32(env, "volumeGroupId", static_cast<int32_t>(action.deviceDescriptors[i]->volumeGroupId_), value);
            SetValueInt32(env, "volumeGroupId_", static_cast<int32_t>(action.deviceDescriptors[i]->volumeGroupId_), value);
