...
static void GetEffectInfoArray(napi_env env, unique_ptr<AudioStreamMgrAsyncContext> &asyncContext, napi_value result)
{
    for (size_t i = PARAM0; i < argc; i++) {
        napi_valuetype valueType = napi_undefined;
        napi_typeof(env, argv[i], &valueType);
...
napi_value AudioStreamMgrNapi::GetEffectInfoArray(napi_env env, napi_callback_info info)
{
    ...
    if ((status == napi_ok && asyncContext->objectInfo != nullptr)
        && (asyncContext->objectInfo->audioStreamMngr_ != nullptr)) {
        GetEffectInfoArray(env, asyncContext, result);
    }
    return result;
}
} // namespace AudioStandard
