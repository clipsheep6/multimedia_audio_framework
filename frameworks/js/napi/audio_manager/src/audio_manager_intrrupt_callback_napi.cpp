...
AudioManagerInterruptCallbackNapi::AudioManagerInterruptCallbackNapi(napi_env env)
    : env_(env)
{
    AUDIO_DEBUG_LOG("AudioManagerInterruptCallbackNapi: instance create");
}

AudioManagerInterruptCallbackNapi::~AudioManagerInterruptCallbackNapi()
{
    AUDIO_DEBUG_LOG("AudioManagerInterruptCallbackNapi: instance destroy");
}

void AudioManagerInterruptCallbackNapi::SaveCallbackReference(const std::string &callbackName, napi_value args)
{
    AUDIO_DEBUG_LOG("AudioManagerInterruptCallbackNapi: SaveCallbackReference");
    std::lock_guard<std::mutex> lock(mutex_);
    napi_ref callback = nullptr;
    const int32_t refCount = 1;
    napi_status status = napi_create_reference(env_, args, refCount, &callback);
    CHECK_AND_RETURN_LOG(status == napi_ok && callback != nullptr,
                         "AudioManagerInterruptCallbackNapi: creating reference for callback fail");
    std::shared_ptr<AutoRef> cb = std::make_shared<AutoRef>(env_, callback);
    if (callbackName == INTERRUPT_CALLBACK_NAME) {
        audioManagerInterruptCallback_ = cb;
    } else {
        AUDIO_ERR_LOG("AudioManagerInterruptCallbackNapi: Unknown callback type: %{public}s", callbackName.c_str());
    }
}

static void SetValueInt32(const napi_env &env, const std::string &fieldStr, const int intValue, napi_value &result)
{
    AUDIO_DEBUG_LOG("AudioManagerInterruptCallbackNapi: SetValueInt32");
    napi_value value;
...
{
    AUDIO_DEBUG_LOG("AudioManagerInterruptCallbackNapi1: SetValueBoolean");
    napi_value value;
...
{
    AUDIO_DEBUG_LOG("AudioManagerInterruptCallbackNapi1: NativeInterruptActionToJsObj");
...
{
    std::lock_guard<std::mutex> lock(mutex_);
    AUDIO_DEBUG_LOG("AudioManagerInterruptCallbackNapi: OnInterrupt");
    AUDIO_DEBUG_LOG("AudioManagerInterruptCallbackNapi: actionType: %{public}d", interruptAction.actionType);
    AUDIO_DEBUG_LOG("AudioManagerInterruptCallbackNapi: interruptType: %{public}d", interruptAction.interruptType);
    AUDIO_DEBUG_LOG("AudioManagerInterruptCallbackNapi: interruptHint: %{public}d", interruptAction.interruptHint);
    AUDIO_DEBUG_LOG("AudioManagerInterruptCallbackNapi: activated: %{public}d", interruptAction.activated);
...
{
    AUDIO_DEBUG_LOG("AudioManagerInterruptCallbackNapi:OnJsCallbackAudioManagerInterrupt");
    ...
    if (work == nullptr) {
        AUDIO_ERR_LOG("AudioManagerInterruptCallbackNapi:OnJsCallbackAudioManagerInterrupt: No memory");
        return;
    }
    if (jsCb.get() == nullptr) {
        AUDIO_ERR_LOG("AudioManagerInterruptCallbackNapi: OnJsCallbackAudioManagerInterrupt: jsCb.get() is null");
        delete work;
        return;
    }
    ...
    int ret = uv_queue_work(loop, work, [] (uv_work_t *work) {}, [] (uv_work_t *work, int status) {
        ...
        AUDIO_DEBUG_LOG("AudioManagerInterruptCallbackNapi: JsCallBack %{public}s, uv_queue_work start",
            request.c_str());
        do {
    ...
    if (ret != 0) {
        AUDIO_ERR_LOG("AudioManagerInterruptCallbackNapi:OnJsCallback: Failed to execute libuv work queue");
        delete work;
    } else {
        jsCb.release();
    }
