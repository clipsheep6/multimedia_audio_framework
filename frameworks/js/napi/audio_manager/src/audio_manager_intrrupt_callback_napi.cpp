...
AudioManagerInterruptCallbackNapi::AudioManagerInterruptCallbackNapi(napi_env env)
    : env_(env)
{
    MEDIA_DEBUG_LOG("AudioManagerInterruptCallbackNapi: instance create");
}

AudioManagerInterruptCallbackNapi::~AudioManagerInterruptCallbackNapi()
{
    MEDIA_DEBUG_LOG("AudioManagerInterruptCallbackNapi: instance destroy");
}
...
void AudioManagerInterruptCallbackNapi::SaveCallbackReference(const std::string &callbackName, napi_value args)
{
    ...
    if (callbackName == INTERRUPT_CALLBACK_NAME) {
        ...
        MEDIA_ERR_LOG("AudioManagerInterruptCallbackNapi: Unknown callback type: %{public}s", callbackName.c_str());
    }
}
...
static void SetValueBoolean(const napi_env& env, const std::string& fieldStr, const bool booValue, napi_value& result)
{
    napi_value value;
    napi_get_boolean(env, booValue, &value);
    napi_set_named_property(env, result, fieldStr.c_str(), value);
}
...
{
    std::lock_guard<std::mutex> lock(mutex_);
    MEDIA_DEBUG_LOG("AudioManagerInterruptCallbackNapi: OnInterrupt is called");
    MEDIA_DEBUG_LOG("AudioManagerInterruptCallbackNapi: actionType: %{public}d", interruptAction.actionType);
    MEDIA_DEBUG_LOG("AudioManagerInterruptCallbackNapi: interruptType: %{public}d", interruptAction.interruptType);
    MEDIA_DEBUG_LOG("AudioManagerInterruptCallbackNapi: interruptHint: %{public}d", interruptAction.interruptHint);
    MEDIA_DEBUG_LOG("AudioManagerInterruptCallbackNapi: activated: %{public}d", interruptAction.activated);
...
void AudioManagerInterruptCallbackNapi::OnJsCallbackAudioManagerInterrupt(std::unique_ptr<AudioManagerInterruptJsCallback> &jsCb)
{
    ...
    if (jsCb.get() == nullptr) {
        MEDIA_ERR_LOG("AudioManagerInterruptCallbackNapi: OnJsCallbackAudioManagerInterrupt: jsCb.get() is null");
        delete work;
        return;
    }
    ...
    int ret = uv_queue_work(loop, work, [] (uv_work_t *work) {}, [] (uv_work_t *work, int status) {
        ...
        MEDIA_DEBUG_LOG("AudioManagerInterruptCallbackNapi: JsCallBack %{public}s, uv_queue_work start", request.c_str());
        do {
