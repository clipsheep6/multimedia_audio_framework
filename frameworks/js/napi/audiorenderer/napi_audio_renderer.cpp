...
{
    AUDIO_INFO_LOG("SetSilentModeAndMixWithOthers");
    napi_value result = nullptr;
    size_t argc = ARGS_ONE;
    napi_value argv[ARGS_ONE] = {};
    auto *napiAudioRenderer = GetParamWithSync(env, info, argc. argv);
    ...
    napi_typeof(env, argv[PARAM0], &valueType);
    CHECK_AND_RETURN_RET_LOG(valueType == napi_number, NapiAudioError::ThrowErrorAndReturn(env, NAPI_ERR_INPUT_INVALID,
        "incorrect parameter types: The type of speed must be number"), "valueType param0 invalid");
...
{
    AUDIO_INFO_LOG("GetSilentModeAndMixWithOthers");
    napi_value result = nullptr;
    size_t argc = ARGS_ONE;
    napi_value argv[ARGS_ONE] = {};
    auto *napiAudioRenderer = GetParamWithSync(env, info, argc. argv);
